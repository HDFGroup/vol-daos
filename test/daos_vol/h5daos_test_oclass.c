...
int test_oclass(hid_t fcpl_id, hid_t fapl_id, const char *exp_root_oclass, const char *exp_group_oclass,
                const char *exp_dset_oclass);
...
check_plist_oclass(hid_t plist_id, const char *exp_oclass)
{
    ...
    if (oclass_size != (ssize_t)strlen(oclass_buf)) {
        H5_FAILED() AT() printf("incorrect object class size\n");
        goto error;
    } /* end if */
    if (strcmp(oclass_buf, exp_oclass)) {
        H5_FAILED()
        AT() printf("returned object class \"%s\" does not match expected: \"%s\"\n", oclass_buf, exp_oclass);
        goto error;
    } /* end if */
 ...
 */
int
test_oclass(hid_t fcpl_id, hid_t fapl_id, const char *exp_root_oclass, const char *exp_group_oclass,
            const char *exp_dset_oclass)
{
    hid_t   file_id  = -1;
    hid_t   obj_id   = -1;
    ...
    if (0 != check_plist_oclass(plist_id, exp_group_oclass)) {
        H5_FAILED() AT() printf("object class check failed for default group\n");
        goto error;
    } /* end if */
    ...
    if (0 != check_plist_oclass(plist_id, "S2")) {
        H5_FAILED() AT() printf("object class check failed for custom group\n");
        goto error;
    } /* end if */
    if (H5Pclose(plist_id) < 0)
        TEST_ERROR
    ...
    if ((obj_id = H5Dcreate2(file_id, DEF_DSET_NAME, H5T_NATIVE_INT, space_id, H5P_DEFAULT, H5P_DEFAULT,
                             H5P_DEFAULT)) < 0)
        TEST_ERROR
...
#else
            (void)
            exp_dset_oclass; /* silence compiler */
#endif
    if (H5Pclose(plist_id) < 0)
        TEST_ERROR
    plist_id = -1;
    ...
    if (H5daos_set_object_class(plist_id, "S1") < 0)
        TEST_ERROR
    if ((obj_id = H5Dcreate2(file_id, MOD_DSET_NAME, H5T_NATIVE_INT, space_id, H5P_DEFAULT, plist_id,
                             H5P_DEFAULT)) < 0)
        TEST_ERROR
    if (H5Pclose(plist_id) < 0)
        TEST_ERROR
    ...
    if (0 != check_plist_oclass(plist_id, "S1")) {
        H5_FAILED() AT() printf("object class check failed for custom dataset\n");
        goto error;
    } /* end if */
    if (H5Pclose(plist_id) < 0)
        TEST_ERROR
    ...
    for (i = 0; i < 2; i++) {
        ...
        if (0 != check_plist_oclass(plist_id, exp_root_oclass)) {
            H5_FAILED() AT() printf("object class check failed for file%s\n", i ? " after file reopen" : "");
            goto error;
        } /* end if */
        if (H5Pclose(plist_id) < 0)
            TEST_ERROR
        ...
        if (0 != check_plist_oclass(plist_id, exp_root_oclass)) {
            H5_FAILED()
            AT() printf("object class check failed for root group%s\n", i ? " after file reopen" : "");
            goto error;
        } /* end if */
        if (H5Pclose(plist_id) < 0)
            TEST_ERROR
        ...
        if (0 != check_plist_oclass(plist_id, exp_group_oclass)) {
            H5_FAILED()
            AT() printf("object class check failed for default group after %s reopen", i ? "file" : "object");
            goto error;
        } /* end if */
        if (H5Pclose(plist_id) < 0)
            TEST_ERROR
        ...
        if (0 != check_plist_oclass(plist_id, "S2")) {
            H5_FAILED()
            AT() printf("object class check failed for custom group after %s reopen", i ? "file" : "object");
            goto error;
        } /* end if */
        if (H5Pclose(plist_id) < 0)
            TEST_ERROR
...
main(int argc, char **argv)
{
    ...
    if (MAINPROCESS)
        puts("All DAOS object class tests passed");
