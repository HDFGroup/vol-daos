    ...
    tse_task_t **first_task, tse_task_t **dep_task)
{
    ...
    if(H5_daos_create_daos_task(DAOS_OPC_OBJ_FETCH, *dep_task ? 1 : 0, *dep_task ? dep_task : NULL,
            H5_daos_md_rw_prep_cb, H5_daos_link_read_comp_cb, read_udata, &read_task) < 0)
    ...
    tse_task_t **first_task, tse_task_t **dep_task)
{
    ...
    /* Create task for link read */
    if(H5_daos_create_daos_task(DAOS_OPC_OBJ_FETCH, *dep_task ? 1 : 0, *dep_task ? dep_task : NULL,
            H5_daos_link_read_ln_prep_cb, H5_daos_link_read_comp_cb, read_udata, &read_task) < 0)
        D_GOTO_ERROR(H5E_LINK, H5E_CANTINIT, FAIL, "can't create task to read link");
...
H5_daos_link_write_task(tse_task_t *task)
{
    ...
    if(((H5_daos_group_t *)udata->md_rw_cb_ud.obj)->gcpl_cache.track_corder) {
        ...
        if(H5_daos_create_task(H5_daos_link_write_end_task, dep_task ? 1 : 0, dep_task ? &dep_task : NULL,
            ...
            D_GOTO_ERROR(H5E_LINK, H5E_CANTINIT, -H5_DAOS_SETUP_ERROR, "can't create end task for link write");
...
H5_daos_link_write_end_task(tse_task_t *task)
{
    ...
    if(H5_daos_link_write_corder_info((H5_daos_group_t *)udata->md_rw_cb_ud.obj, udata->max_corder, udata,
        ...
        D_GOTO_ERROR(H5E_LINK, H5E_CANTINIT, -H5_DAOS_SETUP_ERROR, "can't create task for writing link creation order information to group");
    ...
    tse_task_t **first_task, tse_task_t **dep_task)
{
    ...
    /* Create task to finish this operation */
    if(H5_daos_create_task(H5_daos_link_write_task, *dep_task ? 1 : 0, *dep_task ? dep_task : NULL,
            NULL, NULL, link_write_ud, &link_write_ud->link_write_task) < 0)
...
H5_daos_link_wr_corder_info_task(tse_task_t *task)
{
    ...
    if(H5_daos_create_daos_task(DAOS_OPC_OBJ_UPDATE, 0, NULL, H5_daos_md_rw_prep_cb,
        ...
        D_GOTO_ERROR(H5E_SYM, H5E_CANTINIT, -H5_DAOS_SETUP_ERROR, "can't create task to write link creation order info");
...
H5_daos_link_create_task(tse_task_t *task)
{
    ...
    if (dep_task) {
        ...
        if(H5_daos_create_task(H5_daos_metatask_autocomp_other, 1, &dep_task,
            ...
            D_DONE_ERROR(H5E_LINK, H5E_CANTINIT, -H5_DAOS_SETUP_ERROR, "can't create metatask for link create");
            tse_task_complete(task, ret_value);
	    }

        /* Schedule metatask */
        if(0 != (ret = tse_task_schedule(metatask, false)))
            D_DONE_ERROR(H5E_LINK, H5E_CANTINIT, ret, "can't schedule metatask for link create: %s", H5_daos_err_to_string(ret));
        dep_task = metatask;
    }
    ...
    hid_t lapl_id, hid_t dxpl_id, void **req)
{
    ...
    /* Start H5 operation */
    if(NULL == (int_req = H5_daos_req_create(item->file, "link create",
        ...
        switch(create_args->op_type) {
            case H5VL_LINK_CREATE_HARD:
            {
                ...
                if(H5VL_OBJECT_BY_NAME == target_loc_params_hard->type) {
                    ...
                    if(NULL == (int_int_req = H5_daos_req_create(target_loc_obj_hard->item.file, "target object open within link create",
                    ...
                    if(H5_daos_object_open_helper((H5_daos_item_t *)target_loc_obj_hard, target_loc_params_hard,
                            NULL, FALSE, NULL, &create_udata->target_obj, int_int_req, &first_task, &dep_tasks[ndeps]) < 0)
                    ...
                    if(H5_daos_create_task(H5_daos_h5op_finalize, dep_tasks[ndeps] ? 1 : 0,
                        ...
                        D_GOTO_ERROR(H5E_LINK, H5E_CANTINIT, FAIL, "can't create task to finalize internal operation");
                    ...
                    if(first_task) {
                        if(0 != (ret = tse_task_schedule(int_int_req->finalize_task, false)))
                            D_GOTO_ERROR(H5E_LINK, H5E_CANTINIT, FAIL, "can't schedule task to finalize H5 operation: %s", H5_daos_err_to_string(ret));
                    } /* end if */
                    else
...
H5_daos_link_copy_move_task(tse_task_t *task)
{
    ...
    if(udata->link_val.type == H5L_TYPE_HARD) {
        if(udata->move) {
             ...
             * not be visible to the API, will not be added to an operation
             * pool, and will be integrated into this function's task chain. */
            if(NULL == (int_int_req = H5_daos_req_create(udata->target_obj->item.file, "target object open within link copy/move",
            ...
            if(H5_daos_object_open_helper(&udata->target_obj->item, &link_target_loc_params,
                    NULL, FALSE, NULL, &udata->link_target_obj, int_int_req, &first_task, &dep_tasks[rc_task]) < 0)
            ...
            if(H5_daos_create_task(H5_daos_h5op_finalize, dep_tasks[rc_task] ? 1 : 0,
                    dep_tasks[rc_task] ? &dep_tasks[rc_task] : NULL, NULL, NULL,
                            int_int_req, &int_int_req->finalize_task) < 0)
                D_GOTO_ERROR(H5E_LINK, H5E_CANTINIT, -H5_DAOS_SETUP_ERROR, "can't create task to finalize H5 operation");

            /* Schedule finalize task (or save it to be scheduled later),
             * give it ownership of int_int_req, and update task pointers */
            if(first_task) {
                if(0 != (ret = tse_task_schedule(int_int_req->finalize_task, false)))
                    D_GOTO_ERROR(H5E_LINK, H5E_CANTINIT, ret, "can't schedule task to finalize H5 operation: %s", H5_daos_err_to_string(ret));
            } /* end if */
            else
                first_task = int_int_req->finalize_task;
    ...
    H5_daos_req_t *req, tse_task_t **first_task, tse_task_t **dep_task)
{
    ...
    if(!collective || (req->file->my_rank == 0)) {
        ...
        if(H5_daos_create_task(H5_daos_link_copy_move_task, 2, &dep_tasks[0],
                NULL, NULL, cm_udata, &cm_udata->cm_task) < 0)
        ...
        *dep_task = cm_udata->cm_task;
        ...
        if(delete_task) {
            ...
            /* Create metatask */
            if(H5_daos_create_task(H5_daos_metatask_autocomplete, 2, dep_tasks, NULL, NULL,
                    NULL, &metatask) < 0)
...
H5_daos_link_copy(void *src_item, const H5VL_loc_params_t *loc_params1,
    ...
    hid_t lapl_id, hid_t H5VL_DAOS_UNUSED dxpl_id, void **req)
{
    ...
    /* Start H5 operation */
    if(NULL == (int_req = H5_daos_req_create(src_file, "link copy",
                src_item ? ((H5_daos_item_t *)src_item)->open_req
                : ((H5_daos_item_t *)dst_item)->open_req,
                src_item && dst_item && ((H5_daos_item_t *)src_item)->open_req
                != ((H5_daos_item_t *)dst_item)->open_req
                ? ((H5_daos_item_t *)dst_item)->open_req : NULL,
    ...
    if(int_req) {
         ...
         */
        if(!dst_item
                || ((H5_daos_item_t *)dst_item)->type != H5I_GROUP
                || ((((H5_daos_item_t *)dst_item)->open_req->status == 0
                || ((H5_daos_item_t *)dst_item)->created)
        ...
        if(src_item && dst_item && ((H5_daos_item_t *)src_item)->file
                != ((H5_daos_item_t *)dst_item)->file)
        ...
        if(H5_daos_req_enqueue(int_req, first_task, src_item ? src_item : dst_item,
                op_type, op_scope, collective, !req) < 0)
            D_DONE_ERROR(H5E_LINK, H5E_CANTINIT, FAIL, "can't add request to request queue");
    ...
    hid_t lapl_id, hid_t H5VL_DAOS_UNUSED dxpl_id, void **req)
{
    ...
    /* Start H5 operation */
    if(NULL == (int_req = H5_daos_req_create(src_file, "link move",
                src_item ? ((H5_daos_item_t *)src_item)->open_req
                : ((H5_daos_item_t *)dst_item)->open_req,
                src_item && dst_item && ((H5_daos_item_t *)src_item)->open_req
                != ((H5_daos_item_t *)dst_item)->open_req
    ...
    H5VL_link_get_args_t *get_args, hid_t dxpl_id, void **req)
{
    ...
    /* Start H5 operation */
    if(NULL == (int_req = H5_daos_req_create(item->file, "link get",
            item->open_req, NULL, NULL, dxpl_id)))
        D_GOTO_ERROR(H5E_LINK, H5E_CANTALLOC, FAIL, "can't create DAOS request");
    ...
    if(int_req) {
        ...
        if(req) {
            ...
            *req = int_req;
            ...
            /* Check for failure */
            if(int_req->status < 0)
                D_DONE_ERROR(H5E_LINK, H5E_CANTOPERATE, FAIL, "link get operation failed in task \"%s\": %s", int_req->failed_task, H5_daos_err_to_string(int_req->status));
    ...
    H5VL_link_specific_args_t *specific_args, hid_t dxpl_id, void **req)
{
    ...
    /* Start H5 operation */
    if(NULL == (int_req = H5_daos_req_create(item->file, "link specific",
            item->open_req, NULL, NULL, dxpl_id)))
        D_GOTO_ERROR(H5E_LINK, H5E_CANTALLOC, FAIL, "can't create DAOS request");
        ...
        case H5VL_LINK_ITER:
            {
                ...
                switch (loc_params->type) {
                    ...
                    case H5VL_OBJECT_BY_NAME:
                    {
                         ...
                         * not be visible to the API, will not be added to an operation
                         * pool, and will be integrated into this function's task chain. */
                        if(NULL == (int_int_req = H5_daos_req_create(item->file, "target object open within link iterate by name",
                                NULL, NULL, int_req, H5I_INVALID_HID)))
                        ...
                        if(NULL == (target_grp = (H5_daos_group_t *)H5_daos_group_open_int(item, &sub_loc_params,
                                loc_params->loc_data.loc_by_name.name, H5P_GROUP_ACCESS_DEFAULT, int_int_req,
                        ...
                        if(first_task) {
                            if(0 != (ret = tse_task_schedule(int_int_req->finalize_task, false)))
                                D_GOTO_ERROR(H5E_SYM, H5E_CANTINIT, FAIL, "can't schedule task to finalize H5 operation: %s", H5_daos_err_to_string(ret));
                        } /* end if */
                        else
                            first_task = int_int_req->finalize_task;
...
H5_daos_link_follow_task(tse_task_t *task)
{
    ...
    if(udata->link_read) {
        ...
        if(udata->crt_missing_grp) {
             ...
             * not be visible to the API, will not be added to an operation
             * pool, and will be integrated into this function's task chain. */
            if(NULL == (int_int_req = H5_daos_req_create(udata->grp->obj.item.file, "missing group create within link traversal",
                    NULL, NULL, req, H5I_INVALID_HID)))
                D_GOTO_ERROR(H5E_LINK, H5E_CANTALLOC, -H5_DAOS_ALLOC_ERROR, "can't create DAOS request");
            ...
            if(NULL == (target_grp = (H5_daos_group_t *)H5_daos_group_create_helper(udata->grp->obj.item.file, FALSE,
                    ...
                    udata->name_len, FALSE, int_int_req, &first_task, &dep_task)))
                D_GOTO_ERROR(H5E_SYM, H5E_CANTINIT, -H5_DAOS_SETUP_ERROR, "can't create missing group");

            /* Create task to finalize internal operation */
            if(H5_daos_create_task(H5_daos_h5op_finalize, dep_task ? 1 : 0, dep_task ? &dep_task : NULL,
                    NULL, NULL, int_int_req, &int_int_req->finalize_task) < 0)
                D_GOTO_ERROR(H5E_LINK, H5E_CANTINIT, -H5_DAOS_SETUP_ERROR, "can't create task to finalize internal operation");
    ...
    if(dep_task) {
        ...
        if(H5_daos_create_task(H5_daos_link_follow_end, 1, &dep_task, NULL, NULL,
                udata, &follow_end_task) < 0)
            D_DONE_ERROR(H5E_LINK, H5E_CANTINIT, -H5_DAOS_SETUP_ERROR, "can't create task to finish following link");
        else {
    ...
    hbool_t *link_exists, tse_task_t **first_task, tse_task_t **dep_task)
{
    ...
    assert(*dep_task);
    if(H5_daos_create_task(H5_daos_link_follow_task, 1, dep_task, NULL, NULL,
            follow_udata, &follow_udata->follow_task) < 0)
...
H5_daos_link_get_info_end_task(tse_task_t *task)
{
    ...
    if(udata) {
        ...
        if(dep_task) {
            ...
            if(H5_daos_create_task(H5_daos_metatask_autocomp_other, 1, &dep_task,
                    NULL, NULL, task, &metatask) < 0)
                D_DONE_ERROR(H5E_LINK, H5E_CANTINIT, -H5_DAOS_SETUP_ERROR, "can't create metatask for link get info end");
            else {
                /* Schedule metatask */
                assert(first_task);
                if(0 != (ret = tse_task_schedule(metatask, false)))
    ...
    H5_daos_req_t *req, tse_task_t **first_task, tse_task_t **dep_task)
{
    ...
    switch (loc_params->type) {
        ...
        case H5VL_OBJECT_BY_IDX:
        {
             ...
             * not be visible to the API, will not be added to an operation
             * pool, and will be integrated into this function's task chain. */
            if(NULL == (int_int_req = H5_daos_req_create(item->file, "target object open within link get info by index",
                    NULL, NULL, req, H5I_INVALID_HID)))
                D_GOTO_ERROR(H5E_LINK, H5E_CANTALLOC, -H5_DAOS_ALLOC_ERROR, "can't create DAOS request");
            ...
            if(NULL == (task_udata->target_obj = (H5_daos_obj_t *)H5_daos_group_open_int(item, &sub_loc_params,
                    loc_params->loc_data.loc_by_idx.name, loc_params->loc_data.loc_by_idx.lapl_id, int_int_req, FALSE, first_task, dep_task)))
    ...
    if(task_udata) {
        /* Create task to finish this operation */
        if(H5_daos_create_task(H5_daos_link_get_info_end_task, *dep_task ? 1 : 0, *dep_task ? dep_task : NULL,
            ...
            D_DONE_ERROR(H5E_LINK, H5E_CANTINIT, -H5_DAOS_SETUP_ERROR, "can't create task for link get info end");
        }
        else {
    ...
    tse_task_t **first_task, tse_task_t **dep_task)
{
    ...
    if(task_udata) {
        ...
        if(H5_daos_create_task(H5_daos_link_get_val_end_task, *dep_task ? 1 : 0, *dep_task ? dep_task : NULL,
            ...
            if(*first_task) {
                if(0 != (ret = tse_task_schedule(end_task, false)))
                    D_DONE_ERROR(H5E_LINK, H5E_CANTINIT, FAIL, "can't schedule task for link get val: %s", H5_daos_err_to_string(ret));
            } /* end if */
            else
                *first_task = end_task;
            *dep_task = end_task;
    ...
    H5_daos_req_t *req, tse_task_t **first_task, tse_task_t **dep_task)
{
    ...
    if(H5_daos_create_daos_task(DAOS_OPC_OBJ_FETCH, *dep_task ? 1 : 0, *dep_task ? dep_task : NULL,
            H5_daos_link_exists_prep_cb, H5_daos_link_exists_comp_cb, fetch_udata, &fetch_task) < 0)
        ...
        if(task->dt_result < -H5_DAOS_PRE_ERROR
            ...
            for(i = 0; i < udata->nr; i++) {
                ...
                if(p[0] != '/') {
                    ...
                    if(H5_daos_create_task(H5_daos_link_iterate_op_task, dep_task ? 1 : 0, dep_task ? &dep_task : NULL,
...
H5_daos_link_iterate_op_task(tse_task_t *task)
{
    ...
    if(udata->iter_ud->iter_data->is_recursive) {
        ...
        if((H5L_TYPE_HARD == udata->link_val.type) && (H5I_GROUP == H5_daos_oid_to_type(udata->link_val.target.hard))
            ...
            *oid_lo_copy = udata->link_val.target.hard.lo;
            ...
            if(!ret) {
                DV_free(oid_lo_copy);
                D_GOTO_ERROR(H5E_LINK, H5E_CANTINSERT, -H5_DAOS_SETUP_ERROR, "failed to insert link into visited link table");
            } /* end if */

            /* Start internal H5 operation for target object open.  This will
             * not be visible to the API, will not be added to an operation
             * pool, and will be integrated into this function's task chain. */
            if(NULL == (int_int_req = H5_daos_req_create(udata->iter_ud->target_obj->item.file, "target object open within recursive link iterate",
                    NULL, NULL, req, H5I_INVALID_HID)))
                D_GOTO_ERROR(H5E_LINK, H5E_CANTALLOC, -H5_DAOS_ALLOC_ERROR, "can't create DAOS request");
            ...
            if(NULL == (subgroup = H5_daos_group_open_int(&udata->iter_ud->target_obj->item, &sub_loc_params,
                    udata->link_path, H5P_GROUP_ACCESS_DEFAULT, int_int_req, FALSE, &first_task, &dep_task)))
    ...
    if(dep_task) {
        ...
        if(H5_daos_create_task(H5_daos_link_iter_op_end, 1, &dep_task, NULL, NULL,
                udata, &op_end_task) < 0)
            D_DONE_ERROR(H5E_LINK, H5E_CANTINIT, -H5_DAOS_SETUP_ERROR, "can't create task to finish iteration op");
        else {
...
H5_daos_link_ibco_op_task(tse_task_t *task)
{
    ...
    if(udata->iter_data->is_recursive) {
        ...
        if((H5L_TYPE_HARD == udata->link_val.type) && (H5I_GROUP == H5_daos_oid_to_type(udata->link_val.target.hard))
            ...
            *oid_lo_copy = udata->link_val.target.hard.lo;
             ...
             * not be visible to the API, will not be added to an operation
             * pool, and will be integrated into this function's task chain. */
            if(NULL == (int_int_req = H5_daos_req_create(udata->target_grp->obj.item.file, "target object open within recursive link iterate by creation order",
                    NULL, NULL, req, H5I_INVALID_HID)))
                D_GOTO_ERROR(H5E_LINK, H5E_CANTALLOC, -H5_DAOS_ALLOC_ERROR, "can't create DAOS request");
            ...
            if(NULL == (subgroup = H5_daos_group_open_int(&udata->target_grp->obj.item, &sub_loc_params,
                    udata->link_name, H5P_GROUP_ACCESS_DEFAULT, int_int_req, FALSE, &first_task, &dep_task)))
                D_GOTO_ERROR(H5E_LINK, H5E_CANTOPENOBJ, -H5_DAOS_H5_OPEN_ERROR, "failed to open group");
    ...
    if(++udata->crt_idx < udata->grp_nlinks) {
        ...
        if(H5_daos_create_task(H5_daos_link_ibco_task, dep_task ? 1 : 0, dep_task ? &dep_task : NULL,
                NULL, NULL, udata, &next_task) < 0)
            D_GOTO_ERROR(H5E_LINK, H5E_CANTINIT, -H5_DAOS_SETUP_ERROR, "can't create task for link iterate by creation order");
...
H5_daos_link_ibco_task2(tse_task_t *task)
{
    ...
    /* Create task for iter op */
    if(H5_daos_create_task(H5_daos_link_ibco_op_task, dep_task ? 1 : 0, dep_task ? &dep_task : NULL,
            NULL, NULL, udata, &op_task) < 0)
        D_GOTO_ERROR(H5E_LINK, H5E_CANTINIT, -H5_DAOS_SETUP_ERROR, "can't create task for iteration op");
...
H5_daos_link_ibco_task(tse_task_t *task)
{
    ...
    if(udata->crt_idx < udata->grp_nlinks) {
        ...
        /* Create task to continue this operation */
        if(H5_daos_create_task(H5_daos_link_ibco_task2, dep_task ? 1 : 0, dep_task ? &dep_task : NULL,
                NULL, NULL, udata, &task2_task) < 0)
            D_GOTO_ERROR(H5E_LINK, H5E_CANTINIT, -H5_DAOS_SETUP_ERROR, "can't create task 2 for link iterate by creation order");
    ...
    tse_task_t **first_task, tse_task_t **dep_task)
{
    ...
    if(H5_daos_create_task(H5_daos_link_ibco_end_task, 0, NULL, NULL, NULL,
            ibco_udata, &ibco_udata->ibco_metatask) < 0)
        D_GOTO_ERROR(H5E_LINK, H5E_CANTINIT, -H5_DAOS_SETUP_ERROR, "can't create task for link iterate by creation order");

    /* Create task to continue this operation */
    if(H5_daos_create_task(H5_daos_link_ibco_task, *dep_task ? 1 : 0, *dep_task ? dep_task : NULL,
            NULL, NULL, ibco_udata, &ibco_task) < 0)
        D_GOTO_ERROR(H5E_LINK, H5E_CANTINIT, -H5_DAOS_SETUP_ERROR, "can't create task for link iterate by creation order");
    ...
    if(!collective || (item->file->my_rank == 0)) {
        ...
        if(H5VL_OBJECT_BY_NAME == loc_params->type) {
            ...
            if(NULL == (delete_udata->target_obj = (H5_daos_obj_t *)H5_daos_group_open_int(item,
                    ...
                    int_int_req, collective, first_task, dep_task)))
         ...
         */
        if(H5_daos_create_task(H5_daos_link_delete_corder_pretask, *dep_task ? 1 : 0, *dep_task ? dep_task : NULL,
                NULL, NULL, delete_udata, &delete_pretask) < 0)
        ...
        *dep_task = delete_pretask;
        ...
        if(H5_daos_create_daos_task(DAOS_OPC_OBJ_PUNCH_DKEYS, *dep_task ? 1 : 0, *dep_task ? dep_task : NULL,
                H5_daos_link_delete_prep_cb, H5_daos_link_delete_comp_cb, delete_udata, &delete_task) < 0)
        ...
        *dep_task = delete_task;
        ...
        if(dec_rc) {
            ...
            if(H5_daos_create_task(H5_daos_link_delete_rc_task, *dep_task ? 1 : 0, *dep_task ? dep_task : NULL,
                    NULL, NULL, rc_udata, &rc_udata->rc_task) < 0)
...
H5_daos_link_delete_corder_pretask(tse_task_t *task)
{
    ...
    if(udata) {
        ...
        if(dep_task) {
            ...
            if(H5_daos_create_task(H5_daos_metatask_autocomp_other, 1, &dep_task,
                    NULL, NULL, task, &metatask) < 0)
                D_DONE_ERROR(H5E_LINK, H5E_CANTINIT, -H5_DAOS_SETUP_ERROR, "can't create metatask for link corder info delete task");
            else {
                /* Schedule metatask */
                assert(first_task);
                if(0 != (ret = tse_task_schedule(metatask, false)))
    ...
    const char *target_link_name, H5_daos_req_t *req, tse_task_t **first_task, tse_task_t **dep_task)
{
    ...
    if(H5_daos_create_daos_task(DAOS_OPC_OBJ_UPDATE, *dep_task ? 1 : 0, *dep_task ? dep_task : NULL,
            H5_daos_link_delete_corder_unl_prep_cb, NULL, corder_delete_ud, &update_task) < 0)
    ...
    *dep_task = update_task;
    ...
    if(H5_daos_create_daos_task(DAOS_OPC_OBJ_PUNCH_AKEYS, *dep_task ? 1 : 0, *dep_task ? dep_task : NULL,
            H5_daos_link_delete_corder_prep_cb, NULL, corder_delete_ud, &delete_task) < 0)
    ...
    *dep_task = delete_task;
    ...
    if(H5_daos_create_task(H5_daos_link_delete_corder_bookkeep_task, *dep_task ? 1 : 0,
        ...
        D_GOTO_ERROR(H5E_LINK, H5E_CANTINIT, FAIL, "can't create task to update group's link creation order index");
    ...
    *dep_task = bookkeep_task;
    ...
    if(H5_daos_create_task(H5_daos_link_delete_corder_finish, *dep_task ? 1 : 0,
            *dep_task ? dep_task : NULL, NULL, NULL, corder_delete_ud, &finish_task) < 0)
        D_GOTO_ERROR(H5E_LINK, H5E_CANTINIT, FAIL, "can't create task to update group's link creation order index");
...
H5_daos_link_delete_corder_bookkeep_task(tse_task_t *task)
{
    ...
    if(H5_daos_create_daos_task(DAOS_OPC_OBJ_FETCH, 0, NULL, H5_daos_link_bookkeep_phase1_prep_cb, NULL,
        ...
        D_GOTO_ERROR(H5E_LINK, H5E_CANTINIT, -H5_DAOS_SETUP_ERROR, "can't create task to fetch sizes of link creation order index akeys");
    ...
    if(H5_daos_create_daos_task(DAOS_OPC_OBJ_FETCH, 1, &dep_task, H5_daos_link_bookkeep_phase2_prep_cb, NULL,
        ...
        D_GOTO_ERROR(H5E_LINK, H5E_CANTINIT, -H5_DAOS_SETUP_ERROR, "can't create task to fetch link creation order index akeys");
    ...
    assert(dep_task);
    if(H5_daos_create_daos_task(DAOS_OPC_OBJ_UPDATE, 1, &dep_task, H5_daos_link_bookkeep_phase3_prep_cb, NULL,
            udata, &bookkeeping_tasks[2]) < 0)
        D_GOTO_ERROR(H5E_LINK, H5E_CANTINIT, -H5_DAOS_SETUP_ERROR, "can't create task to update link creation order index akeys");
    ...
    assert(dep_task);
    if(H5_daos_create_daos_task(DAOS_OPC_OBJ_PUNCH_AKEYS, 1, &dep_task, H5_daos_link_bookkeep_phase4_prep_cb, NULL,
            udata, &bookkeeping_tasks[3]) < 0)
        D_GOTO_ERROR(H5E_LINK, H5E_CANTINIT, -H5_DAOS_SETUP_ERROR, "can't create task to remove invalid index akeys");
    ...
    if(udata) {
        ...
        if(dep_task) {
            ...
            if(H5_daos_create_task(H5_daos_metatask_autocomp_other, 1, &dep_task,
                    NULL, NULL, task, &metatask) < 0) {
                D_DONE_ERROR(H5E_LINK, H5E_CANTINIT, -H5_DAOS_SETUP_ERROR, "can't create metatask for link creation order index update task");
                tse_task_complete(task, ret_value);
            }
            else {
                /* Schedule metatask */
...
H5_daos_link_delete_rc_task(tse_task_t *task)
{
    ...
    if(udata->link_val.type == H5L_TYPE_HARD) {
         ...
         * not be visible to the API, will not be added to an operation
         * pool, and will be integrated into this function's task chain. */
        if(NULL == (int_int_req = H5_daos_req_create(udata->target_obj->item.file, "target object open for link delete ref count decrement",
                NULL, NULL, req, H5I_INVALID_HID)))
            D_GOTO_ERROR(H5E_LINK, H5E_CANTALLOC, -H5_DAOS_ALLOC_ERROR, "can't create DAOS request");

        /* Open the link target object */
        if(H5_daos_object_open_helper(&udata->target_obj->item, &link_target_loc_params,
                NULL, FALSE, NULL, &udata->link_target_obj, int_int_req, &first_task, &dep_task) < 0)
    ...
    if(udata) {
        ...
        /* Create task to free udata */
        if(H5_daos_create_task(H5_daos_link_delete_rc_end_task, dep_task ? 1 : 0, dep_task ? &dep_task : NULL,
                NULL, NULL, udata, &end_task) < 0) {
...
H5_daos_link_gnbc_task(tse_task_t *task)
{
    ...
    if(H5_daos_create_daos_task(DAOS_OPC_OBJ_FETCH, 0, NULL, H5_daos_md_rw_prep_cb,
        ...
        D_GOTO_ERROR(H5E_LINK, H5E_CANTINIT, -H5_DAOS_SETUP_ERROR, "can't create task to read link name by creation order");
    ...
    H5_daos_req_t *req, tse_task_t **first_task, tse_task_t **dep_task)
{
    ...
    /* Create task to finish this operation */
    if(H5_daos_create_task(H5_daos_link_gnbc_task, *dep_task ? 1 : 0, *dep_task ? dep_task : NULL,
            NULL, NULL, gnbc_udata, &gnbc_udata->gnbc_task) < 0)
...
H5_daos_link_gnbn_task(tse_task_t *task)
{
    ...
    if(udata) {
        ...
        if(H5_daos_create_task(H5_daos_link_gnbn_end_task, dep_task ? 1 : 0, dep_task ? &dep_task : NULL,
                NULL, NULL, udata, &end_task) < 0)
            D_DONE_ERROR(H5E_LINK, H5E_CANTINIT, -H5_DAOS_SETUP_ERROR, "can't create task for link get name by name order end");
        else {
    ...
    H5_daos_req_t *req, tse_task_t **first_task, tse_task_t **dep_task)
{
    ...
    /* Create task to finish this operation */
    if(H5_daos_create_task(H5_daos_link_gnbn_task, *dep_task ? 1 : 0, *dep_task ? dep_task : NULL,
            NULL, NULL, gnbn_udata, &gnbn_udata->gnbn_task) < 0)
    ...
    uint64_t *crt_order, H5_daos_req_t *req, tse_task_t **first_task, tse_task_t **dep_task)
{
    ...
    if(H5_daos_create_daos_task(DAOS_OPC_OBJ_FETCH, *dep_task ? 1 : 0, *dep_task ? dep_task : NULL,
            H5_daos_md_rw_prep_cb, H5_daos_link_gcbn_comp_cb, fetch_udata, &fetch_task) < 0)
