...
{
    char *daos_sys = NULL;
    hid_t file = -1, dset = -1, dset_a = -1, dset_b = -1, dset_c = -1, dset2 = -1, attr = -1, attr_a = -1,
          attr_b = -1, attr_c = -1, space = -1, space2 = -1, space2_contig = -1, fapl = -1;
    ...
    const type_all buf_init[4] = {
        {-1, 'a', (double)-1.}, {-2, 'b', (double)-2.}, {-3, 'c', (double)-3.}, {-4, 'd', (double)-4.}};
    const type_convall buf_conv_init[4] = {{(long long)-5, 'e', (float)-5.},
                                           {(long long)-6, 'f', (float)-6.},
                                           {(long long)-7, 'g', (float)-7.},
                                           {(long long)-8, 'h', (float)-8.}};
    const type_convall buf2_init[4][2]  = {
        {{(long long)-1, 'a', (float)-1.}, {(long long)-2, 'b', (float)-2.}},
        {{(long long)-3, 'c', (float)-3.}, {(long long)-4, 'd', (float)-4.}},
        {{(long long)-5, 'e', (float)-5.}, {(long long)-6, 'f', (float)-6.}},
        {{(long long)-7, 'g', (float)-7.}, {(long long)-8, 'h', (float)-8.}}};
    size_t i, j;
    int    i2;
    ...
    for (i = 0; (hsize_t)i < dims[0]; i++) {
        ...
        if (((file_buf[i].c - buf[i].c) > (double)0.01) || ((file_buf[i].c - buf[i].c) < (double)-0.01))
    ...
    for (i = 0; (hsize_t)i < dims[0]; i++) {
        ...
        if (buf_conv_init[i].b != buf_conv[i].b)
            PRINTF_ERROR("Member b at location %zu does not match", i);
        if (((buf_conv_init[i].c - buf_conv[i].c) > 0.01f) || ((buf_conv_init[i].c - buf_conv[i].c) < -0.01f))
            PRINTF_ERROR("Member c at location %zu does not match", i);
    } /* end for */
        ...
        for (i = 0; (hsize_t)i < dims[0]; i++) {
            ...
            for (j = 0; j < dims[1]; j++) {
                if (j > 0)
                    printf(", ");
                printf("{%lld, %c, %.1f}",
                       (i == 1 || i == 2) ? (long long)file_buf2[i][j].a : buf2_init[i][j].a,
                       buf2_init[i][j].b, (double)buf2_init[i][j].c);
            } /* end for */
        }     /* end for */
        printf("}\n");
        printf("buf = {");
    ...
    for (i = 0; (hsize_t)i < dims[0]; i++)
        for (j = 0; j < dims[1]; j++) {
            ...
            if ((j == 1) || (i == 0) || (i == 3))
                /*! ----------------------------------------------*/
                if (((i == 1 || i == 2) ? (long long)file_buf2[i][j].a : buf2_init[i][j].a) != buf2[i][j].a)
                    PRINTF_ERROR("Member a at location %zu, %zu does not match", i, j);
            if (buf2_init[i][j].b != buf2[i][j].b)
                PRINTF_ERROR("Member b at location %zu, %zu does not match", i, j);
            if (((buf2_init[i][j].c - buf2[i][j].c) > 0.01f) || ((buf2_init[i][j].c - buf2[i][j].c) < -0.01f))
                PRINTF_ERROR("Member c at location %zu, %zu does not match", i, j);
        } /* end for */
        ...
        for (i = 0; (hsize_t)i < dims[0]; i++) {
            ...
            for (j = 0; j < dims[1]; j++) {
                if (j > 0)
                    printf(", ");
                printf("{%lld, %c, %.1f}",
                       (j == 1) ? (long long)file_buf2[i / 2 + 1][i % 2].a : buf2_init[i][j].a,
                       buf2_init[i][j].b, (double)buf2_init[i][j].c);
            } /* end for */
        }     /* end for */
        printf("}\n");
    ...
    for (i = 0; (hsize_t)i < dims[0]; i++)
        for (j = 0; j < dims[1]; j++) {
            ...
            if ((j == 0) || (i == 1) || (i == 3))
                /*! ----------------------------------------------*/
                if (((j == 1) ? (long long)file_buf2[i / 2 + 1][i % 2].a : buf2_init[i][j].a) != buf2[i][j].a)
                    PRINTF_ERROR("Member a at location %zu, %zu does not match", i, j);
            if (buf2_init[i][j].b != buf2[i][j].b)
                PRINTF_ERROR("Member b at location %zu, %zu does not match", i, j);
            if (((buf2_init[i][j].c - buf2[i][j].c) > 0.01f) || ((buf2_init[i][j].c - buf2[i][j].c) < -0.01f))
