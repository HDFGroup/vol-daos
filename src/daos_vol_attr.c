...
static herr_t H5_daos_attribute_open_by_idx_helper(H5_daos_obj_t *target_obj,
    const H5VL_loc_params_t *loc_params, H5_daos_attr_t *attr_out, H5_daos_req_t *req,
...
static herr_t H5_daos_attribute_write_int(H5_daos_attr_t *attr,
    hid_t mem_type_id, hbool_t collective, htri_t need_tconv, const void *buf,
    H5_daos_attr_io_ud_t *_udata, H5_daos_req_t *req, tse_task_t **first_task,
    tse_task_t **dep_task);
...
static herr_t H5_daos_attribute_get_name(H5_daos_obj_t *target_obj, const H5VL_loc_params_t *loc_params,
    char *attr_name_out, size_t attr_name_out_size, size_t *size_ret, H5_daos_req_t *req,
    tse_task_t **first_task, tse_task_t **dep_task);
...
static herr_t H5_daos_attribute_get_name_by_idx(H5_daos_obj_t *target_obj, H5_index_t index_type,
    ...
    size_t *attr_name_size, H5_daos_req_t *req, tse_task_t **first_task, tse_task_t **dep_task);
    ...
    hid_t aapl_id, hid_t H5VL_DAOS_UNUSED dxpl_id, void H5VL_DAOS_UNUSED **req)
{
    ...
    /* Start H5 operation */
    if(NULL == (int_req = H5_daos_req_create(item->file, "attribute create",
    ...
    tse_task_t **first_task, tse_task_t **dep_task)
{
    ...
    if(loc_params->type == H5VL_OBJECT_BY_SELF) {
        ...
        if(NULL == (int_int_req = H5_daos_req_create(item->file, "target object open within attribute create by name",
        ...
        if(*first_task) {
            if(0 != (ret = tse_task_schedule(int_int_req->finalize_task, false)))
                D_GOTO_ERROR(H5E_ATTR, H5E_CANTINIT, NULL, "can't schedule task to finalize H5 operation: %s", H5_daos_err_to_string(ret));
        } /* end if */
        else
    ...
    if(!collective || (item->file->my_rank == 0)) {
        ...
        daos_const_iov_set((d_const_iov_t *)&create_ud->md_rw_cb_ud.dkey, H5_daos_attr_key_g, H5_daos_attr_key_size_g);
    ...
    H5_daos_req_t *req, tse_task_t **first_task, tse_task_t **dep_task)
{
     ...
     */
    daos_const_iov_set((d_const_iov_t *)&create_ud->md_rw_cb_ud.iod[3].iod_name,
    ...
    void H5VL_DAOS_UNUSED **req)
{
    ...
    /* Start H5 operation */
    if(NULL == (int_req = H5_daos_req_create(item->file, "attribute open",
            item->open_req, NULL, NULL, H5I_INVALID_HID)))
        D_GOTO_ERROR(H5E_ATTR, H5E_CANTALLOC, NULL, "can't create DAOS request");
    ...
    H5_daos_req_t *req, tse_task_t **first_task, tse_task_t **dep_task)
{
     ...
     * info buffer size */
    if(collective && (item->file->num_procs > 1)) {
        if(NULL == (bcast_udata = (H5_daos_attr_ibcast_ud_t *)DV_malloc(sizeof(H5_daos_attr_ibcast_ud_t) + H5_DAOS_AINFO_BCAST_BUF_SIZE)))
        ...
        bcast_udata->bcast_ud.comm = req->file->facc_params.comm;
    ...
    switch (loc_params->type) {
        ...
        case H5VL_OBJECT_BY_NAME:
        {
             ...
             * not be visible to the API, will not be added to an operation
             * pool, and will be integrated into this function's task chain. */
            if(NULL == (int_int_req = H5_daos_req_create(item->file, "target object open within attribute open by name",
                    NULL, NULL, req, H5I_INVALID_HID)))
                D_GOTO_ERROR(H5E_ATTR, H5E_CANTALLOC, NULL, "can't create DAOS request");
        ...
        case H5VL_OBJECT_BY_IDX:
        {
            ...
            H5_DAOS_WAIT_ON_ASYNC_CHAIN(req, *first_task, *dep_task,
    ...
    if(bcast_udata) {
        if(H5_daos_mpi_ibcast((H5_daos_mpi_ibcast_ud_t *)bcast_udata, NULL,
    ...
    tse_task_t **first_task, tse_task_t **dep_task)
{
     ...
     * not be visible to the API, will not be added to an operation
     * pool, and will be integrated into this function's task chain. */
    if(NULL == (int_int_req = H5_daos_req_create(target_obj->item.file, "target object open within attribute create by name",
            NULL, NULL, req, H5I_INVALID_HID)))
    ...
    if(H5_daos_object_open_helper((H5_daos_item_t *)target_obj, &sub_loc_params, NULL, TRUE,
            NULL, &attr_parent_obj, int_int_req, first_task, dep_task) < 0)
    ...
    tse_task_t **dep_task)
{
    ...
    if(need_tconv) {
        ...
        if(H5_daos_tconv_init(attr->file_type_id, &file_type_size,
                mem_type_id, &mem_type_size, (size_t)attr_nelmts, FALSE, FALSE,
                &tconv_buf, &bkg_buf, &reuse, &fill_bkg) < 0)
    ...
    if(!collective || (attr->item.file->my_rank == 0) || need_tconv) {
        ...
        if(!collective || (attr->item.file->my_rank == 0)) {
            ...
            /* Set up dkey */
            daos_const_iov_set((d_const_iov_t *)&udata->md_rw_cb_ud.dkey, H5_daos_attr_key_g, H5_daos_attr_key_size_g);
    ...
    hid_t dxpl_id, void H5VL_DAOS_UNUSED **req)
{
    ...
    if(NULL == (int_req = H5_daos_req_create(attr->item.file, "attribute read",
            attr->item.open_req, NULL, NULL, req_dxpl_id)))
        D_GOTO_ERROR(H5E_ATTR, H5E_CANTALLOC, FAIL, "can't create DAOS request");
    ...
    if(int_req) {
        ...
        if(H5_daos_req_enqueue(int_req, first_task, &attr->item,
                H5_DAOS_OP_TYPE_READ, H5_DAOS_OP_SCOPE_ATTR, collective, !req) < 0)
...
H5_daos_attribute_read_comp_cb(tse_task_t *task, void H5VL_DAOS_UNUSED *args)
{
    ...
    if(task->dt_result < -H5_DAOS_PRE_ERROR
        ...
        if(udata->md_rw_cb_ud.sgl[0].sg_nr_out == 0)
            (void)memset(udata->md_rw_cb_ud.sg_iov[0].iov_buf, 0, (size_t)udata->attr_nelmts * udata->file_type_size);
 ...
 */
herr_t
H5_daos_attribute_write_int(H5_daos_attr_t *attr, hid_t mem_type_id,
    hbool_t collective, htri_t need_tconv, const void *buf,
    H5_daos_attr_io_ud_t *_udata, H5_daos_req_t *req, tse_task_t **first_task,
    tse_task_t **dep_task)
{
    H5_daos_attr_io_ud_t *udata = _udata;
    ...
    if(need_tconv) {
        /* Initialize type conversion */
        if(H5_daos_tconv_init(mem_type_id, &mem_type_size, attr->file_type_id, &file_type_size, (size_t)attr_nelmts, FALSE, TRUE, &tconv_buf, &bkg_buf, NULL, &fill_bkg) < 0)
            D_GOTO_ERROR(H5E_ATTR, H5E_CANTINIT, FAIL, "can't initialize type conversion");
    } /* end if */
    else
        file_type_size = attr->file_type_size;
        ...
        if(fill_bkg) {
            ...
            if(*first_task) {
                if(0 != (ret = tse_task_schedule(bkg_fill_task, false)))
                    D_GOTO_ERROR(H5E_ATTR, H5E_CANTINIT, FAIL, "can't schedule fetch task for background buffer read: %s", H5_daos_err_to_string(ret));
            } /* end if */
            else
    ...
    if(*first_task) {
        if(0 != (ret = tse_task_schedule(update_task, false)))
            D_GOTO_ERROR(H5E_ATTR, H5E_CANTINIT, FAIL, "can't schedule fetch task for attribute write: %s", H5_daos_err_to_string(ret));
    } /* end if */
    else
        *first_task = update_task;
    *dep_task = update_task;
    ...
    hid_t dxpl_id, void H5VL_DAOS_UNUSED **req)
{
    ...
    /* Start H5 operation.  Currently the DXPL is only copies when we may
     * perform type conversion. */
    if(NULL == (int_req = H5_daos_req_create(attr->item.file, "attribute write",
            attr->item.open_req, NULL, NULL, req_dxpl_id)))
        D_GOTO_ERROR(H5E_ATTR, H5E_CANTALLOC, FAIL, "can't create DAOS request");
     ...
     * like HDF5, metadata writes are collective by default.
     */
    H5_DAOS_GET_METADATA_WRITE_MODE(attr->item.file, H5P_ATTRIBUTE_ACCESS_DEFAULT, H5P_ATTRIBUTE_ACCESS_DEFAULT,
    ...
    if(attr->item.open_req->status == 0) {
        /* Call internal routine */
        if(H5_daos_attribute_write_int(attr, mem_type_id, collective, need_tconv,
                buf, NULL, int_req, &first_task, &dep_task) < 0)
    ...
    if(int_req) {
         ...
         * dependency on the attribute open if necessary. */
        if(H5_daos_req_enqueue(int_req, first_task, &attr->item,
                H5_DAOS_OP_TYPE_WRITE, H5_DAOS_OP_SCOPE_ATTR, collective, !req) < 0)
            D_DONE_ERROR(H5E_ATTR, H5E_CANTINIT, FAIL, "can't add request to request queue");
        ...
        if(req) {
            ...
            *req = int_req;
            ...
            /* Check for failure */
            if(int_req->status < 0)
                D_DONE_ERROR(H5E_ATTR, H5E_CANTOPERATE, FAIL, "attribute write failed in task \"%s\": %s", int_req->failed_task, H5_daos_err_to_string(int_req->status));
...
H5_daos_attribute_read_bkg_comp_cb(tse_task_t *task, void H5VL_DAOS_UNUSED *args)
{
    ...
    if(task->dt_result < -H5_DAOS_PRE_ERROR
        ...
        /* Perform type conversion */
        if(H5Tconvert(udata->mem_type_id, udata->attr->file_type_id, udata->attr_nelmts,
                udata->tconv_buf, udata->bkg_buf, udata->md_rw_cb_ud.req->dxpl_id) < 0)
            D_GOTO_ERROR(H5E_ATTR, H5E_CANTCONVERT, -H5_DAOS_H5_TCONV_ERROR, "can't perform type conversion");
        ...
        case H5VL_ATTR_GET_NAME:
            {
                ...
                /* Start H5 operation */
                if(NULL == (int_req = H5_daos_req_create(item->file, "get attribute name",
                        item->open_req, NULL, NULL, H5I_INVALID_HID)))
                    D_GOTO_ERROR(H5E_ATTR, H5E_CANTALLOC, FAIL, "can't create DAOS request");
        ...
        case H5VL_ATTR_GET_INFO:
            {
                ...
                /* Start H5 operation */
                if(NULL == (int_req = H5_daos_req_create(item->file, "get attribute info",
                        item->open_req, NULL, NULL, H5I_INVALID_HID)))
                    D_GOTO_ERROR(H5E_ATTR, H5E_CANTALLOC, FAIL, "can't create DAOS request");
    ...
    va_list arguments)
{
    ...
    /* Start H5 operation */
    if(NULL == (int_req = H5_daos_req_create(item->file, "attribute specific",
            item->open_req, NULL, NULL, H5I_INVALID_HID)))
        D_GOTO_ERROR(H5E_ATTR, H5E_CANTALLOC, FAIL, "can't create DAOS request");
    ...
    if(!target_obj) {
         ...
         * not be visible to the API, will not be added to an operation
         * pool, and will be integrated into this function's task chain. */
        if(NULL == (int_int_req = H5_daos_req_create(item->file, "target object open within attribute specific operation",
                NULL, NULL, int_req, H5I_INVALID_HID)))
            D_GOTO_ERROR(H5E_ATTR, H5E_CANTALLOC, FAIL, "can't create DAOS request");

        /* Open target_obj */
        if(H5_daos_object_open_helper(item, sub_loc_params_p, NULL, collective_md_read,
                NULL, &target_obj, int_int_req, &first_task, &dep_task) < 0)
        ...
        H5_DAOS_WAIT_ON_ASYNC_CHAIN(int_req, first_task, dep_task,
        ...
        case H5VL_ATTR_ITER:
            {
                ...
                collective = FALSE;
                op_type = H5_DAOS_OP_TYPE_READ;
                if(H5_daos_attribute_iterate(target_obj, &iter_data,
                        int_req, &first_task, &dep_task) < 0)
...
H5_daos_attribute_close(void *_attr, hid_t H5VL_DAOS_UNUSED dxpl_id, void **req)
{
    ...
    if(((attr->item.open_req->status == 0
            || attr->item.open_req->status < -H5_DAOS_CANCELED)
        ...
        if(NULL == (int_req = H5_daos_req_create(attr->item.file, "attribute close",
                attr->item.open_req, NULL, NULL, H5P_DATASET_XFER_DEFAULT)))
            D_GOTO_ERROR(H5E_ATTR, H5E_CANTALLOC, FAIL, "can't create DAOS request");
    ...
    if(int_req) {
         ...
         * dependency on the attribute open if necessary. */
        if(H5_daos_req_enqueue(int_req, first_task, &attr->item,
                H5_DAOS_OP_TYPE_CLOSE, H5_DAOS_OP_SCOPE_ATTR, FALSE, !req) < 0)
            D_DONE_ERROR(H5E_ATTR, H5E_CANTINIT, FAIL, "can't add request to request queue");
        attr = NULL;
    ...
    H5_daos_req_t *req, tse_task_t **first_task, tse_task_t **dep_task)
{
    ...
    switch (loc_params->type) {
        ...
        case H5VL_OBJECT_BY_IDX:
        {
             ...
             * not be visible to the API, will not be added to an operation
             * pool, and will be integrated into this function's task chain. */
            if(NULL == (int_int_req = H5_daos_req_create(target_obj->item.file, "target object open within attribute get name by index",
                    NULL, NULL, req, H5I_INVALID_HID)))
            ...
            sub_loc_params.loc_data.loc_by_name.lapl_id = loc_params->loc_data.loc_by_idx.lapl_id;
            if(H5_daos_object_open_helper((H5_daos_item_t *)target_obj, &sub_loc_params, NULL, TRUE,
                    NULL, &parent_obj, int_int_req, first_task, dep_task) < 0)
            ...
            if(*first_task) {
                if(0 != (ret = tse_task_schedule(int_int_req->finalize_task, false)))
                    D_GOTO_ERROR(H5E_ATTR, H5E_CANTINIT, (-1), "can't schedule task to finalize H5 operation: %s", H5_daos_err_to_string(ret));
            } /* end if */
            else
                *first_task = int_int_req->finalize_task;
            *dep_task = int_int_req->finalize_task;
    ...
    H5_daos_req_t *req, tse_task_t **first_task, tse_task_t **dep_task)
{
    ...
    if(H5_daos_attribute_get_info_inplace(get_info_udata, prep_cb, comp_cb,
            req, first_task, dep_task) < 0)
    ...
    if(!collective || (attr_container_obj->item.file->my_rank == 0)) {
        ...
        if(attr_container_obj->ocpl_cache.track_acorder) {
            ...
            if(H5_daos_object_update_num_attrs_key(delete_udata->attr_parent_obj, &delete_udata->cur_num_attrs,
                    NULL, NULL, req, first_task, dep_task) < 0)
    ...
    H5_daos_req_t *req, tse_task_t **first_task, tse_task_t **dep_task)
{
    ...
    if((obj_nattrs_remaining > 0) && (delete_idx < (uint64_t)obj_nattrs_remaining)) {
        ...
        daos_const_iov_set((d_const_iov_t *)&iod.iod_name, H5_daos_max_attr_corder_key_g, H5_daos_max_attr_corder_key_size_g);
        iod.iod_nr = 1u;
    ...
    tse_task_t **first_task, tse_task_t **dep_task)
{
    ...
    if(collective && (attr_container_obj->item.file->num_procs > 1)) {
        ...
        attr_exists_ud->bcast_ud.comm = req->file->facc_params.comm;
        must_bcast = TRUE;
    } /* end if */
    ...
    if(!collective || (attr_container_obj->item.file->my_rank == 0)) {
        ...
        /* Set up dkey */
        daos_const_iov_set((d_const_iov_t *)&attr_exists_ud->dkey, H5_daos_attr_key_g, H5_daos_attr_key_size_g);
    ...
    if(collective && (attr_container_obj->item.file->num_procs > 1))
        if(H5_daos_mpi_ibcast(&attr_exists_ud->bcast_ud, attr_container_obj, sizeof(htri_t),
                FALSE, NULL, H5_daos_attr_exists_bcast_comp_cb, req, first_task,
                dep_task) < 0)
    ...
    if(attr_exists_ud) {
        ...
        if(must_bcast) {
            ...
            *((htri_t *)attr_exists_ud->bcast_ud.buffer) = FAIL;
            if(H5_daos_mpi_ibcast(&attr_exists_ud->bcast_ud, attr_container_obj,
                    sizeof(htri_t), FALSE, NULL, H5_daos_attr_exists_bcast_comp_cb,
                    req, first_task, dep_task) < 0)
...
H5_daos_attr_exists_comp_cb(tse_task_t *task, void H5VL_DAOS_UNUSED *args)
{
    ...
    if(udata) {
        ...
        if(udata->bcast_ud.obj &&
                H5_daos_object_close(&udata->bcast_ud.obj->item) < 0)
            D_DONE_ERROR(H5E_ATTR, H5E_CLOSEERROR, -H5_DAOS_H5_CLOSE_ERROR, "can't close object");
    ...
    H5_daos_req_t *req, tse_task_t **first_task, tse_task_t **dep_task)
{
    ...
    if(H5_daos_object_get_num_attrs(iterate_udata->attr_container_obj, &iterate_udata->u.crt_order_data.obj_nattrs,
            FALSE, req, first_task, dep_task) < 0)
    ...
    for(cur_idx = 0; cur_idx < (uint64_t)iterate_udata->u.crt_order_data.obj_nattrs; cur_idx++) {
        ...
        /* Create task to call user-supplied operator callback function */
        if(H5_daos_attribute_get_iter_op_task(iterate_udata, target_attr_name, req, first_task, dep_task) < 0)
    ...
    *dep_task = int_int_req->finalize_task;
    ...
    if(!collective || (attr_container_obj->item.file->my_rank == 0)) {
        ...
        *dep_task = int_int_req->finalize_task;
        ...
        if(H5_daos_attribute_write_int(new_attr, new_attr->type_id, FALSE,
                -1, attr_data_buf, NULL, req, first_task, dep_task) < 0)
...
H5_daos_attr_gnbi_alloc_task(tse_task_t *task)
{
    ...
    if(*udata->attr_name_size > udata->cur_attr_name_size - 1) {
        ...
        if(H5_daos_attribute_get_name_by_idx(udata->target_obj, udata->index_type,
                ...
                udata->attr_name_size, udata->req,
    ...
    tse_task_t **dep_task)
{
    ...
    if(H5_daos_attribute_get_name_by_idx(target_obj, index_type, iter_order, idx,
            *gnbi_udata->attr_name_buf, gnbi_udata->cur_attr_name_size,
            gnbi_udata->attr_name_size, req, first_task, dep_task) < 0)
...
H5_daos_attribute_get_name_by_idx(H5_daos_obj_t *target_obj, H5_index_t index_type,
    H5_iter_order_t iter_order, uint64_t idx, char *attr_name_out, size_t attr_name_out_size,
    size_t *attr_name_size, H5_daos_req_t *req, tse_task_t **first_task,
    tse_task_t **dep_task)
{
