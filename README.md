...
HDF5 DAOS VOL plugin version 1.0.0 - currently under development
...
### Table of Contents:
    ...
    II. Installation
        ...
        B. Building the DAOS VOL plugin
            ...
            iii. Manual Build
                ...
                b. Options for CMake
            iv. Build Results
        C. Testing the DAOS VOL plugin installation
    III. Using the DAOS VOL plugin
        A. Writing HDF5 DAOS VOL plugin applications
            i. Skeleton Example
        B. Building HDF5 DAOS VOL plugin applications
        C. Running HDF5 DAOS VOL plugin applications
...
The DAOS VOL plugin is a plugin for HDF5 designed with the goal of allowing
...
The DAOS VOL plugin is under active development, and details given here may
change.
...
Before building and using the HDF5 DAOS VOL plugin, a few requirements must be met.

### II.A.i. External Libraries

To build the DAOS VOL plugin, the following libraries are required:
...
supplied to the DAOS VOL plugin's build scripts. Refer to section II.B.ii. below
for more information.


## II.B. Building the DAOS VOL plugin

### II.B.i. Obtaining the Source

The latest and most up-to-date DAOS VOL plugin code can be viewed at:
...
A source distribution of HDF5 has been included in the DAOS VOL plugin source
...
a version which contains the VOL abstraction layer; otherwise, the DAOS VOL plugin will
...
The following configuration options are available to all of the build scripts:
    ...
    -d      Enables debugging information printouts within the DAOS VOL plugin.

    -m      Enables memory usage tracking within the DAOS VOL plugin. This option is
            mostly useful in helping to diagnose any possible memory leaks or other
            memory errors within the plugin.

    -t      Build the HDF5 tools with DAOS VOL plugin support.
            WARNING: This option is experimental and should not currently be used.

    -P DIR  Specifies where the DAOS VOL plugin should be installed. The default
            installation prefix is `daos_vol_build` inside the DAOS VOL plugin source
            root directory.
...
Additionally, the CMake build scripts have the following configuration option:
    ...
    -B DIR  Specifies the directory that CMake should use as the build tree location.
            ...
            DAOS VOL plugin source root directory. Note that the DAOS VOL does not
...
In general, the process for building the DAOS VOL plugin involves either obtaining a VOL-enabled
HDF5 distribution or building one from source. Then, the DAOS VOL plugin is built using that
...
respective build system in order to build the DAOS VOL plugin against the HDF5 distribution.
...
When building the DAOS VOL plugin manually using Autotools, the following options are
...
The options in the supplied Autotools build script are mapped to the corresponding options here:
    ...
    --enable-build-mode=(production|debug)
                    ...
                    Debug - enable debugging printouts within the DAOS VOL plugin.
                    Production - Focus more on optimization.

    --enable-mem-tracking
                    Enables memory tracking within the DAOS VOL plugin. This option is
                    mostly useful in helping to diagnose any possible memory leaks or
                    other memory errors within the plugin.

    --enable-tests
                    Enables/Disables building of the DAOS VOL plugin tests.

    --enable-examples
                    Enables/Disables building of the DAOS VOL HDF5 examples.

    --enable-tools
                    Enables/Disables building of the HDF5 tools with DAOS VOL plugin
...
When building the DAOS VOL plugin manually using CMake, the following options are available.

Some of the options in the supplied CMake build script are mapped to the corresponding options here:

    CMAKE_INSTALL_PREFIX (Default: `daos_vol_build` in DAOS VOL plugin source root
                          directory)
    ...
    PREBUILT_HDF5_DIR (Default: empty)
                    ...
                    the DAOS VOL plugin's CMake build will attempt to build the
                    ...
                    use that HDF5 distribution to build the DAOS VOL plugin and
    ...
    DAOS_VOL_ENABLE_DEBUG (Default: OFF)
                    Enables/Disables debugging printouts within the DAOS VOL plugin.

    DAOS_VOL_ENABLE_MEM_TRACKING (Default: OFF)
                    Enables/Disables memory tracking withing the DAOS VOL plugin. This
...
these files are placed in `daos_vol_build` in the DAOS VOL plugin source root directory.
...
If the DAOS VOL plugin was built using one of the included build scripts, all of the usual files
...
## II.C. Testing the DAOS VOL plugin installation

The DAOS VOL plugin tests require access to an HDF5 DAOS API-aware server -- see section
II.A.ii and III.C.

After building the DAOS VOL plugin and obtaining access to a server which implements
...
the HDF5 library and DAOS VOL plugin are functioning correctly.

Each of these commands will run the `test_daos_vol` executable, which is built by
each of the DAOS VOL plugin's build systems and contains a set of tests to cover a
...
applications with the DAOS VOL plugin.



## III.A. Writing HDF5 DAOS VOL plugin applications

Any HDF5 application using the DAOS VOL plugin must:
...
+ Link against `libdaosvol.a` (or similar), found in the `lib` directory of the
  DAOS VOL plugin installation directory.
...
Below is a no-op application that opens and closes a file using the DAOS VOL plugin.
...
## III.B. Building HDF5 DAOS VOL plugin applications
...
If the DAOS VOL plugin was built using Autotools, it is highly recommended that compilation
of HDF5 DAOS VOL plugin applications be done using the supplied `h5pcc` script, as it will
...
## III.C. Running HDF5 DAOS VOL plugin applications

Running applications that use the HDF5 DAOS VOL plugin requires access to a server which implements
the DAOS API (); see section II.A.ii.

### III.C.i. Runtime Environment

For the DAOS VOL plugin to correctly interact with an DAOS API-aware server instance, there are two
...
The file `test/test_daos_vol.c`, in addition to being the source for the DAOS VOL plugin
...
of HDF5's public API functionality with the DAOS VOL plugin and should be a good indicator
of whether the DAOS VOL plugin is working correctly in conjunction with a running HDF5 DAOS
API-aware instance.

In addition to this file, some of the example C applications included with HDF5
distributions have been adapted to work with the DAOS VOL plugin and are included
under the top-level `examples` directory in the DAOS VOL plugin source root directory.

--------------------------------------------------------------------------------

# IV. Feature Support

Not all aspects of HDF5 are implemented by or are applicable to the DAOS VOL plugin.
...
known to be problematic for the DAOS VOL plugin and will likely cause issues
