 ...
 *              August 2018
 *
 * Purpose: Tests the DAOS VOL plugin
 */
...
/* Uncomment to print out extra messages during testing */
/* #define DAOS_VOL_DEBUG */
...
/* TODO: Comment out once reference tests are reimplemented for
 * the DAOS VOL.
 */
#define NO_REF_TESTS

/*
 * TODO: Remove once bug in DAOS with multiple daos_init()/fini() calls is resolved.
 */
#define DAOS_INIT_FINI_BUG_WORKAROUND

#define H5A_EXISTS_WORKAROUND

#define HARD_LINK_WORKAROUND

#define EXTERNAL_LINK_WORKAROUND
...
#ifndef EXTERNAL_LINK_WORKAROUND
...
#ifndef PROBLEMATIC_TESTS
    H5E_BEGIN_TRY {
        err_ret = H5Fis_accessible(filename, fapl_id);
    } H5E_END_TRY;
...
test_unused_dataset_API_calls(void)
{
    ...
    H5E_BEGIN_TRY {
        err_ret = H5Dget_storage_size(dset_id);
    } H5E_END_TRY;
 ...
 * test the DAOS VOL plugin's support for datatypes
 */
static hid_t
...
main( int argc, char** argv )
{
    ...
    if (NULL == (pool_string = getenv("DAOS_POOL"))) {
        if (MAINPROCESS) printf("DAOS_POOL is not set! Tests cannot proceed.\n\n");
        goto error;
    }

    if (NULL == (pool_svcl_string = getenv("DAOS_SVCL"))) {
        if (MAINPROCESS) printf("DAOS_SVCL is not set! Tests cannot proceed.\n\n");
        goto error;
    }

    snprintf(filename, FILENAME_MAX_LENGTH, "%s", TEST_FILE_NAME);
    ...
    if (0 != uuid_parse(pool_string, pool_uuid)) {
        if (MAINPROCESS) printf("Could not parse pool UUID\n\n");
        goto error;
    }
    ...
    if (MAINPROCESS) puts("All DAOS VOL plugin tests passed");
