        ...
        if(req) {
            ...
            *req = int_req;
            ...
            if(H5_daos_task_wait(int_req->finalize_task, NULL) < 0)
                D_DONE_ERROR(H5E_SYM, H5E_CANTINIT, NULL, "can't wait for request to finish");
    ...
    if(int_req) {
        ...
        if(req) {
            ...
            *req = int_req;
            ...
            /* Block until operation completes */
            if(H5_daos_task_wait(int_req->finalize_task, NULL) < 0)
                D_DONE_ERROR(H5E_SYM, H5E_CANTINIT, FAIL, "can't wait for request to finish");
    ...
    if(int_req) {
        ...
        if(req) {
            ...
            *req = int_req;
            ...
            /* Check for failure */
            if(int_req->status < 0)
                D_DONE_ERROR(H5E_SYM, H5E_CANTOPERATE, FAIL, "dataset specific operation failed in task \"%s\": %s", int_req->failed_task, H5_daos_err_to_string(int_req->status));
