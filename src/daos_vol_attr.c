        ...
        if(req) {
            ...
            *req = int_req;
            ...
            if(H5_daos_task_wait(int_req->finalize_task, NULL) < 0)
                D_DONE_ERROR(H5E_ATTR, H5E_CANTINIT, NULL, "can't wait for request to finish");
    ...
    if(int_req) {
        ...
        if(req) {
            ...
            *req = int_req;
            ...
            /* Block until operation completes */
            if(H5_daos_task_wait(int_req->finalize_task, NULL) < 0)
                D_DONE_ERROR(H5E_ATTR, H5E_CANTINIT, FAIL, "can't wait for request to finish");
...
H5_daos_attr_exists_prep_cb(tse_task_t *task, void H5VL_DAOS_UNUSED *args)
{
    ...
    /* Handle errors */
    if(udata->bcast_ud.req->status < -H5_DAOS_SHORT_CIRCUIT) {
        udata = NULL;
        D_GOTO_DONE(-H5_DAOS_PRE_ERROR);
    } /* end if */
    else if(udata->bcast_ud.req->status == -H5_DAOS_SHORT_CIRCUIT) {
        udata = NULL;
        D_GOTO_DONE(-H5_DAOS_SHORT_CIRCUIT);
    } /* end if */
