    ...
    hid_t dapl_id, hid_t dxpl_id, void **req)
{
    ...
    if(!collective || (item->file->my_rank == 0)) {
        ...
        /* Seet dataset oid */
        if(oid_ptr)
            /* Retarget *oid_ptr so H5_daos_link_follow fills in the dataset's
             * oid */
        ...
        assert(dep_task);
        if(0 != (ret = daos_task_create(DAOS_OPC_OBJ_FETCH, &item->file->sched, 1, &dep_task, &fetch_task)))
    ...
    hid_t file_space_id, hid_t dxpl_id, void *buf, void H5VL_DAOS_UNUSED **req)
{
    ...
    if(int_req) {
        ...
        /* Register dependencies (if any) */
        else if(ntasks > 0 && 0 != (ret = tse_task_register_deps(int_req->finalize_task, ntasks, finalize_deps)))
