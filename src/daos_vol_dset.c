...
#if H5VL_VERSION >= 2
static herr_t H5_daos_dataset_get_realize(void *future_object,
...
static herr_t H5_daos_dataset_set_extent(H5_daos_dset_t *dset,
    const hsize_t *size, hbool_t collective, H5_daos_req_t *req,
...
H5_daos_dataset_create(void *_item,
    ...
    hid_t dapl_id, hid_t H5VL_DAOS_UNUSED dxpl_id, void **req)
{
    ...
    if(int_req) {
        ...
        if(req) {
            ...
            *req = int_req;
            ...
            /* Check for failure */
            if(int_req->status < 0)
                D_DONE_ERROR(H5E_DATASET, H5E_CANTOPERATE, NULL, "dataset create failed in task \"%s\": %s", int_req->failed_task, H5_daos_err_to_string(int_req->status));
    ...
    tse_task_t **first_task, tse_task_t **dep_task)
{
    ...
    if(!collective || (file->my_rank == 0)) {
        ...
        if(NULL == (update_cb_ud = (H5_daos_md_rw_cb_ud_flex_t *)DV_calloc(sizeof(H5_daos_md_rw_cb_ud_flex_t) + type_size + space_size + dcpl_size + fill_val_size)))
        ...
        daos_const_iov_set((d_const_iov_t *)&update_cb_ud->md_rw_cb_ud.dkey, H5_daos_int_md_key_g, H5_daos_int_md_key_size_g);
        update_cb_ud->md_rw_cb_ud.free_dkey = FALSE;

        /* Set up iod */
        daos_const_iov_set((d_const_iov_t *)&update_cb_ud->md_rw_cb_ud.iod[0].iod_name, H5_daos_type_key_g, H5_daos_type_key_size_g);
        ...
        daos_const_iov_set((d_const_iov_t *)&update_cb_ud->md_rw_cb_ud.iod[1].iod_name, H5_daos_space_key_g, H5_daos_space_key_size_g);
        ...
        daos_const_iov_set((d_const_iov_t *)&update_cb_ud->md_rw_cb_ud.iod[2].iod_name, H5_daos_cpl_key_g, H5_daos_cpl_key_size_g);
        ...
        if(dset->dcpl_cache.fill_status == H5D_FILL_VALUE_USER_DEFINED) {
            ...
            /* Set up iod */
            daos_const_iov_set((d_const_iov_t *)&update_cb_ud->md_rw_cb_ud.iod[update_cb_ud->md_rw_cb_ud.nr].iod_name, H5_daos_fillval_key_g, H5_daos_fillval_key_size_g);
            ...
            /* Set up sgl */
            daos_iov_set(&update_cb_ud->md_rw_cb_ud.sg_iov[update_cb_ud->md_rw_cb_ud.nr], fill_val_buf, (daos_size_t)fill_val_size);
            ...
            update_cb_ud->md_rw_cb_ud.sgl[update_cb_ud->md_rw_cb_ud.nr].sg_iovs = &update_cb_ud->md_rw_cb_ud.sg_iov[update_cb_ud->md_rw_cb_ud.nr];
        ...
        if(parent_grp) {
            ...
            finalize_deps[finalize_ndeps] = *dep_task;
            if(0 != (ret = H5_daos_obj_write_rc(NULL, &dset->obj, NULL, 0,
                    req, first_task, &finalize_deps[finalize_ndeps])))
    ...
    if(0 != (ret = tse_task_create(H5_daos_metatask_autocomplete, &H5_daos_glob_sched_g, NULL, &dataset_metatask)))
    ...
    if(NULL == ret_value) {
        ...
        /* Free memory */
        if(update_cb_ud && update_cb_ud->md_rw_cb_ud.obj && H5_daos_object_close(&update_cb_ud->md_rw_cb_ud.obj->item) < 0)
            D_DONE_ERROR(H5E_DATASET, H5E_CLOSEERROR, NULL, "can't close object");
...
H5_daos_dset_open_bcast_comp_cb(tse_task_t *task, void H5VL_DAOS_UNUSED *args)
{
    ...
    if(udata) {
        /* Close dataset */
        if(udata->bcast_udata.obj && H5_daos_dataset_close_real((H5_daos_dset_t *)udata->bcast_udata.obj) < 0)
            D_DONE_ERROR(H5E_DATASET, H5E_CLOSEERROR, -H5_DAOS_H5_CLOSE_ERROR, "can't close dataset");
...
H5_daos_dset_open_recv_comp_cb(tse_task_t *task, void H5VL_DAOS_UNUSED *args)
{
    ...
    if(task->dt_result < -H5_DAOS_PRE_ERROR
        ...
        if(dinfo_len > (size_t)udata->bcast_udata.count) {
            ...
            /* Open dataset */
            if(0 != (ret = daos_obj_open(udata->bcast_udata.obj->item.file->coh, udata->bcast_udata.obj->oid, udata->bcast_udata.obj->item.file->flags & H5F_ACC_RDWR ? DAOS_COO_RW : DAOS_COO_RO, &udata->bcast_udata.obj->obj_oh, NULL /*event*/)))
            ...
            if(0 != (ret = H5_daos_dset_open_end((H5_daos_dset_t *)udata->bcast_udata.obj,
...
H5_daos_dinfo_read_comp_cb(tse_task_t *task, void H5VL_DAOS_UNUSED *args)
{
    ...
    if(task->dt_result == -DER_REC2BIG) {
        ...
        if(udata->bcast_udata) {
            ...
            if(daos_info_len > H5_DAOS_TYPE_BUF_SIZE + H5_DAOS_SPACE_BUF_SIZE
                    + H5_DAOS_DCPL_BUF_SIZE + H5_DAOS_FILL_VAL_BUF_SIZE) {
                if(NULL == (udata->bcast_udata->bcast_udata.buffer = DV_malloc(daos_info_len + 6 * H5_DAOS_ENCODED_UINT64_T_SIZE)))
                    D_GOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, -H5_DAOS_ALLOC_ERROR, "can't allocate buffer for serialized dataset info");
                udata->bcast_udata->bcast_udata.buffer_len = (int)(daos_info_len + 6 * H5_DAOS_ENCODED_UINT64_T_SIZE);
            } /* end if */
    ...
    if(udata) {
        /* Close dataset */
        if(udata->md_rw_cb_ud.obj && H5_daos_dataset_close_real((H5_daos_dset_t *)udata->md_rw_cb_ud.obj) < 0)
            D_DONE_ERROR(H5E_DATASET, H5E_CLOSEERROR, -H5_DAOS_H5_CLOSE_ERROR, "can't close dataset");

        if(udata->bcast_udata) {
            /* Clear broadcast buffer if there was an error */
            if(udata->md_rw_cb_ud.req->status < -H5_DAOS_INCOMPLETE)
                (void)memset(udata->bcast_udata->bcast_udata.buffer, 0, (size_t)udata->bcast_udata->bcast_udata.count);
        } /* end if */
    ...
    hid_t dapl_id, hid_t dxpl_id, void H5VL_DAOS_UNUSED **req)
{
    ...
    if(NULL == ret_value) {
        ...
        if(must_bcast && H5_daos_mpi_ibcast(NULL, &dset->obj,
    ...
    H5_daos_req_t *req, tse_task_t **first_task, tse_task_t **dep_task)
{
     ...
     * info buffer size */
    if(collective && (file->num_procs > 1)) {
        if(NULL == (bcast_udata = (H5_daos_mpi_ibcast_ud_flex_t *)DV_malloc(sizeof(H5_daos_mpi_ibcast_ud_flex_t) + H5_DAOS_DINFO_BCAST_BUF_SIZE)))
    ...
    if(!collective || (file->my_rank == 0)) {
        ...
        daos_const_iov_set((d_const_iov_t *)&fetch_udata->md_rw_cb_ud.iod[0].iod_name, H5_daos_type_key_g, H5_daos_type_key_size_g);
    ...
    if(bcast_udata) {
        if(H5_daos_mpi_ibcast(&bcast_udata->bcast_udata, &dset->obj, dinfo_buf_size,
    ...
    H5_daos_io_type_t io_type, void *buf, H5_daos_req_t *req, tse_task_t **first_task, tse_task_t **dep_task)
{
    ...
    *p++ = (uint8_t)'\0';
    ...
    if(io_type == IO_READ) {
        ...
        if(0 != (ret = daos_task_create(DAOS_OPC_OBJ_FETCH, &H5_daos_glob_sched_g, *dep_task ? 1 : 0, *dep_task ? dep_task : NULL, &io_task)))
        ...
        if(0 != (ret = daos_task_create(DAOS_OPC_OBJ_UPDATE, &H5_daos_glob_sched_g, *dep_task ? 1 : 0, *dep_task ? dep_task : NULL, &io_task)))
    ...
    *p++ = (uint8_t)'\0';
    ...
    if(io_type == IO_READ) {
        ...
        if(chunk_io_ud->tconv.fill_bkg) {
            ...
            /* Create task to read data from dataset */
            if(0 != (ret = daos_task_create(DAOS_OPC_OBJ_FETCH, &H5_daos_glob_sched_g, *dep_task ? 1 : 0, *dep_task ? dep_task : NULL, &fill_bkg_task)))
    ...
    hid_t file_space_id, hid_t dxpl_id, void *buf, void **req)
{
    ...
    if((dset->obj.item.open_req->status == 0)
        ...
        if(0 != (ret = tse_task_create(H5_daos_dset_io_int_end_task, &H5_daos_glob_sched_g, task_ud, &task_ud->end_task)))
        ...
        if(0 != (ret = tse_task_create(H5_daos_dset_io_int_task, &H5_daos_glob_sched_g, task_ud, &io_task)))
...
H5_daos_dataset_get_task(tse_task_t *task)
{
    ...
    *udata->hsize_out = 0;
    ...
    if(H5I_INVALID_HID == udata->dset->space_id || H5I_INVALID_HID == udata->dset->type_id)
        D_GOTO_ERROR(H5E_DATASET, H5E_CANTGET, -H5_DAOS_BAD_VALUE, "can't get dataset's dataspace or datatype");

    /* Return the in-memory size of the data */
    if((nelements = H5Sget_simple_extent_npoints(udata->dset->space_id)) < 0)
        D_GOTO_ERROR(H5E_DATASPACE, H5E_CANTGET, -H5_DAOS_H5_GET_ERROR, "can't get number of elements in dataset's dataspace");
    ...
    hid_t H5VL_DAOS_UNUSED dxpl_id, void H5VL_DAOS_UNUSED **req, va_list arguments)
{
        ...
        case H5VL_DATASET_GET_DCPL:
            {
                ...
                if(!dset->obj.item.created && dset->obj.item.open_req->status != 0) {
                    ...
                    if((*plist_id = H5Iregister_future(H5I_GENPROP_LST, get_udata,
        ...
        case H5VL_DATASET_GET_SPACE:
            {
                ...
                if(!dset->obj.item.created && dset->obj.item.open_req->status != 0) {
                    ...
                    if((*ret_id = H5Iregister_future(H5I_DATASPACE, get_udata,
                            H5_daos_dataset_get_realize, H5_daos_dataset_get_discard)) < 0)
                        D_GOTO_ERROR(H5E_DATASET, H5E_CANTREGISTER, FAIL, "can't register future ID");
                } /* end if */
                else
        ...
        case H5VL_DATASET_GET_TYPE:
            {
                ...
                if(!dset->obj.item.created && dset->obj.item.open_req->status != 0) {
                    ...
                    if((*ret_id = H5Iregister_future(H5I_DATATYPE, get_udata,
                            H5_daos_dataset_get_realize, H5_daos_dataset_get_discard)) < 0)
                        D_GOTO_ERROR(H5E_DATASET, H5E_CANTREGISTER, FAIL, "can't register future ID");
                } /* end if */
                else
    ...
    if(get_udata) {
        ...
        if(0 != (ret = tse_task_create(get_type == H5VL_DATASET_GET_STORAGE_SIZE
                ? H5_daos_dataset_get_task : H5_daos_metatask_autocomplete,
                &H5_daos_glob_sched_g, get_udata, &get_task)))
            D_GOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL, "can't create task to perform get operation: %s", H5_daos_err_to_string(ret));
    ...
    if(task->dt_result == -DER_REC2BIG) {
        ...
        if(udata->bcast_udata) {
            ...
            if(daos_info_len > H5_DAOS_SPACE_BUF_SIZE) {
                ...
                if(NULL == (udata->bcast_udata->bcast_udata.buffer = DV_malloc(daos_info_len + H5_DAOS_ENCODED_UINT64_T_SIZE)))
                    D_GOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, -H5_DAOS_ALLOC_ERROR, "can't allocate buffer for serialized dataspace info");
                udata->bcast_udata->bcast_udata.buffer_len = (int)daos_info_len + H5_DAOS_ENCODED_UINT64_T_SIZE;
            } /* end if */
    ...
    H5_daos_req_t *req, tse_task_t **first_task, tse_task_t **dep_task)
{
    ...
    /* Set up iod */
    daos_const_iov_set((d_const_iov_t *)&fetch_udata->md_rw_cb_ud.iod[0].iod_name, H5_daos_space_key_g, H5_daos_space_key_size_g);
    fetch_udata->md_rw_cb_ud.iod[0].iod_nr = 1u;
    fetch_udata->md_rw_cb_ud.iod[0].iod_size = DAOS_REC_ANY;
    fetch_udata->md_rw_cb_ud.iod[0].iod_type = DAOS_IOD_SINGLE;
    ...
    tse_task_t **dep_task)
{
    ...
    /* Allocate task udata */
    if(NULL == (update_cb_ud = (H5_daos_dset_set_extent_ud_t *)DV_calloc(sizeof(H5_daos_dset_set_extent_ud_t))))
        D_GOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL, "can't allocate space for dataset set extent udata struct");
    update_cb_ud->md_rw_cb_ud.req = req;
    update_cb_ud->md_rw_cb_ud.obj = &dset->obj;
    ...
    if(!collective || (dset->obj.item.file->my_rank == 0)) {
        ...
        /* Set up iod */
        daos_const_iov_set((d_const_iov_t *)&update_cb_ud->md_rw_cb_ud.iod[0].iod_name, H5_daos_space_key_g, H5_daos_space_key_size_g);
        update_cb_ud->md_rw_cb_ud.iod[0].iod_nr = 1u;
        ...
        if(0 != (ret = tse_task_register_cbs(update_task, H5_daos_md_rw_prep_cb, NULL, 0, H5_daos_dset_set_extent_comp_cb, NULL, 0)))
        ...
        if(0 != (ret = tse_task_create(H5_daos_metatask_autocomplete, &H5_daos_glob_sched_g, update_cb_ud, &update_task)))
            D_GOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL, "can't create task to perform get operation: %s", H5_daos_err_to_string(ret));

        /* Set callback functions for object update */
        if(0 != (ret = tse_task_register_cbs(update_task, NULL, NULL, 0, H5_daos_dset_set_extent_comp_cb, NULL, 0)))
            D_GOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL, "can't register callbacks for task to update object: %s", H5_daos_err_to_string(ret));
    } /* end else */
    ...
    *dep_task = update_task;
    ...
    if(collective && (dset->obj.item.file->num_procs > 1))
        if(H5_daos_collective_error_check(&dset->obj, req, first_task, dep_task)
                < 0)
