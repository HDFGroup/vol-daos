...
main(int argc, char *argv[])
{
    ...
    if ((map1 = H5Mcreate(file, "MAP_VL_T", H5T_NATIVE_INT, dtid1, H5P_DEFAULT, H5P_DEFAULT)) < 0)
        ERROR;
    if ((map2 = H5Mcreate(file, "MAP_VL_STR", H5T_NATIVE_INT, dtid2, H5P_DEFAULT, H5P_DEFAULT)) < 0)
        ERROR;
    if ((map3 = H5Mcreate(file, "MAP_FIXED", H5T_NATIVE_INT, H5T_NATIVE_INT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
        ERROR;

    for (i = 0; i < 5; i++) {
        ret = H5Mset(map1, H5T_NATIVE_INT, &i, dtid1, &wdata[i], H5P_DEFAULT);
        if (ret != 0)
            ERROR;
    } /* end for */

    for (i = 0; i < 5; i++) {
        ret = H5Mset(map2, H5T_NATIVE_INT, &i, dtid2, str_wdata[i], H5P_DEFAULT);
        if (ret != 0)
            ERROR;
    } /* end for */
    ...
    for (i = 0; i < NUM_KEYS; i++) {
        value = 1000 + i;
        ret   = H5Mset(map3, H5T_NATIVE_INT, &i, H5T_NATIVE_INT, &value, H5P_DEFAULT);
        if (ret != 0)
            ERROR;
    } /* end for */
    ...
    for (i = 0; i < 5; i++) {
        ...
        ret = H5Mget(map1, H5T_NATIVE_INT, &i, dtid1, &rdata[i], H5P_DEFAULT);
        if (ret != 0)
            ERROR;
    ...
    for (i = 0; i < 5; i++) {
        str_rdata[i] = NULL;
        ret          = H5Mget(map2, H5T_NATIVE_INT, &i, dtid2, &str_rdata[i], H5P_DEFAULT);
        if (ret != 0)
            ERROR;
        printf("Key %d:  %s\n", i, str_rdata[i]);
        free(str_rdata[i]);
