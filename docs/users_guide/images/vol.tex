...
\node[connector, fill=rulecolor!30, right=50pt of vol_xxx] (vol_daos) {\rotatebox{90}{DAOS}};
\node[layer2, below=20pt of vol_native] (mpio) {MPI I/O};
\node[layer2, fill=rulecolor!30, below=20pt of vol_daos] (daos) {DAOS Client};

\node[storage, below=20pt of mpio] (fs) {File\\ System};
\node[storage, fill=rulecolor!30, below=20pt of daos] (storage) {DAOS\\ Server};


\draw[trans, ->] (app) -- (hdf5);
\draw[trans, ->] (hdf5) -- (vol);
\draw[trans, ->] (vol) -- (vol_native);
\draw[trans, ->] (vol) -- (vol_daos);
\draw[trans, ->] (vol_native) -- (mpio);
\draw[trans, ->] (vol_daos) -- (daos);
\draw[trans, ->] (mpio) -- (fs);
\draw[trans, ->] (mpio) -- (storage);
\draw[trans, ->] (daos) -- (storage);

\draw[dashed] ($(mpio.south west)+(-2,-0.3)$) -- node[right=160pt, font=\small\sffamily\itshape] {Network} ($(daos.south east)+(2,-0.3)$);
\node[below=-15pt of vol_xxx, font=\small\sffamily] (connectors) {VOL connectors};

\node[font=\small\sffamily\bfseries, text width=180pt, text centered, right=10pt of vol_daos] () {\texttt{H5Pset\_fapl\_daos()} \\or\\ \texttt{export HDF5\_VOL\_CONNECTOR=daos}};
