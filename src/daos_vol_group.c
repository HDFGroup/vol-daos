    ...
    tse_task_t **first_task, tse_task_t **dep_task)
{
    ...
    if(!collective || (file->my_rank == 0)) {
        ...
        if(NULL == (update_cb_ud = (H5_daos_md_rw_cb_ud_flex_t *)DV_calloc(sizeof(H5_daos_md_rw_cb_ud_flex_t) + gcpl_size)))
        ...
        daos_const_iov_set((d_const_iov_t *)&update_cb_ud->md_rw_cb_ud.iod[0].iod_name, H5_daos_cpl_key_g, H5_daos_cpl_key_size_g);
    ...
    void **req)
{
    ...
    /* Start H5 operation */
    if(NULL == (int_req = H5_daos_req_create(item->file, "group create", item->open_req,
            NULL, NULL, H5I_INVALID_HID)))
        D_GOTO_ERROR(H5E_SYM, H5E_CANTALLOC, NULL, "can't create DAOS request");
    ...
    if(task->dt_result < -H5_DAOS_PRE_ERROR
        ...
        if(udata->bcast_udata.buffer_len != udata->bcast_udata.count) {
            ...
            if(H5_daos_create_task(H5_daos_mpi_ibcast_task, 0, NULL, NULL, H5_daos_group_open_bcast_comp_cb,
                    udata, &bcast_task) < 0)
                D_GOTO_ERROR(H5E_SYM, H5E_CANTINIT, -H5_DAOS_SETUP_ERROR, "can't create task for second group info broadcast");
...
H5_daos_ginfo_read_comp_cb(tse_task_t *task, void H5VL_DAOS_UNUSED *args)
{
    ...
    if(task->dt_result == -DER_REC2BIG) {
        ...
        if(udata->bcast_udata) {
            ...
            if(NULL == (udata->bcast_udata->bcast_udata.buffer = DV_malloc(udata->md_rw_cb_ud.iod[0].iod_size + 3 * H5_DAOS_ENCODED_UINT64_T_SIZE)))
                D_GOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, -H5_DAOS_ALLOC_ERROR, "can't allocate buffer for serialized group info");
            udata->bcast_udata->bcast_udata.buffer_len = (int)(udata->md_rw_cb_ud.iod[0].iod_size + 3 * H5_DAOS_ENCODED_UINT64_T_SIZE);

            /* Set up sgl */
            daos_iov_set(&udata->md_rw_cb_ud.sg_iov[0], (uint8_t *)udata->bcast_udata->bcast_udata.buffer + 3 * H5_DAOS_ENCODED_UINT64_T_SIZE, udata->md_rw_cb_ud.iod[0].iod_size);
        ...
        if(H5_daos_create_daos_task(DAOS_OPC_OBJ_FETCH, 0, NULL, H5_daos_md_rw_prep_cb,
            ...
            D_GOTO_ERROR(H5E_SYM, H5E_CANTINIT, -H5_DAOS_SETUP_ERROR, "can't create task to read group metadata");
    ...
    const char *name, hid_t gapl_id, hid_t dxpl_id, void H5VL_DAOS_UNUSED **req)
{
    ...
    /* Start H5 operation */
    if(NULL == (int_req = H5_daos_req_create(item->file, "group open", item->open_req,
            NULL, NULL, dxpl_id)))
        D_GOTO_ERROR(H5E_SYM, H5E_CANTALLOC, NULL, "can't create DAOS request");
    ...
    if(int_req) {
        ...
        if(req) {
            ...
            *req = int_req;
            ...
            /* Check for failure */
            if(int_req->status < 0)
                D_DONE_ERROR(H5E_SYM, H5E_CANTOPERATE, FAIL, "group specific operation failed in task \"%s\": %s", int_req->failed_task, H5_daos_err_to_string(int_req->status));
...
H5_daos_group_close(void *_grp, hid_t H5VL_DAOS_UNUSED dxpl_id, void **req)
{
     ...
     * nonblocking. */
    if(((grp->obj.item.open_req->status == 0
            || grp->obj.item.open_req->status < -H5_DAOS_CANCELED)
        ...
        if(NULL == (int_req = H5_daos_req_create(grp->obj.item.file, "group close",
                grp->obj.item.open_req, NULL, NULL, H5P_DATASET_XFER_DEFAULT)))
            D_GOTO_ERROR(H5E_SYM, H5E_CANTALLOC, FAIL, "can't create DAOS request");
    ...
    if(int_req) {
        ...
        if(req) {
            ...
            *req = int_req;
            ...
            /* Check for failure */
            if(int_req->status < 0)
                D_DONE_ERROR(H5E_SYM, H5E_CANTOPERATE, FAIL, "group close failed in task \"%s\": %s", int_req->failed_task, H5_daos_err_to_string(int_req->status));
    ...
    if(udata) {
        ...
        if(dep_task) {
            ...
            if(H5_daos_create_task(H5_daos_metatask_autocomp_other, 1, &dep_task,
                ...
                D_DONE_ERROR(H5E_SYM, H5E_CANTINIT, -H5_DAOS_SETUP_ERROR, "can't create metatask for group get info");
            else {
                /* Schedule metatask */
                assert(first_task);
    ...
    tse_task_t **dep_task)
{
    ...
    switch (loc_params->type) {
        ...
        case H5VL_OBJECT_BY_NAME:
        {
             ...
             * not be visible to the API, will not be added to an operation
             * pool, and will be integrated into this function's task chain. */
            if(NULL == (int_int_req = H5_daos_req_create(grp->obj.item.file, "target object open within group get info by name",
                    NULL, NULL, req, H5I_INVALID_HID)))
                D_GOTO_ERROR(H5E_SYM, H5E_CANTALLOC, FAIL, "can't create DAOS request");
            ...
            if(NULL == (task_udata->target_obj = (H5_daos_obj_t *)H5_daos_group_open_int(&grp->obj.item, &sub_loc_params,
                    loc_params->loc_data.loc_by_name.name, H5P_GROUP_ACCESS_DEFAULT, int_int_req, FALSE,
                    first_task, dep_task)))
        ...
        case H5VL_OBJECT_BY_IDX:
        {
             ...
             * not be visible to the API, will not be added to an operation
             * pool, and will be integrated into this function's task chain. */
            if(NULL == (int_int_req = H5_daos_req_create(grp->obj.item.file, "target object open within group get info by index",
                    NULL, NULL, req, H5I_INVALID_HID)))
                D_GOTO_ERROR(H5E_SYM, H5E_CANTALLOC, FAIL, "can't create DAOS request");

            /* Open target object */
            if(H5_daos_object_open_helper(&grp->obj.item, loc_params, &task_udata->opened_type,
                    FALSE, NULL, &task_udata->target_obj, int_int_req, first_task, dep_task) < 0)
                D_GOTO_ERROR(H5E_SYM, H5E_CANTOPENOBJ, FAIL, "can't open group");
    ...
    if(H5_daos_create_task(H5_daos_group_get_info_task, *dep_task ? 1 : 0, *dep_task ? dep_task : NULL,
            NULL, NULL, task_udata, &get_info_task) < 0)
...
H5_daos_group_gnl_task(tse_task_t *task)
{
    ...
    if(((H5_daos_group_t *)udata->md_rw_cb_ud.obj)->gcpl_cache.track_corder) {
        ...
        if(dep_task) {
            ...
            if(H5_daos_create_task(H5_daos_metatask_autocomp_other, 1, &dep_task,
                    NULL, NULL, task, &metatask) < 0)
                D_GOTO_ERROR(H5E_SYM, H5E_CANTINIT, -H5_DAOS_SETUP_ERROR, "can't create metatask for group get num links");
    ...
    H5_daos_req_t *req, tse_task_t **first_task, tse_task_t **dep_task)
{
    ...
    /* Create task to finish this operation */
    if(H5_daos_create_task(H5_daos_group_gnl_task, *dep_task ? 1 : 0, *dep_task ? dep_task : NULL,
            NULL, NULL, gnl_udata, &gnl_udata->gnl_task) < 0)
    ...
    tse_task_t **dep_task)
{
    ...
    if(H5_daos_create_daos_task(DAOS_OPC_OBJ_FETCH, *dep_task ? 1 : 0, *dep_task ? dep_task : NULL,
            H5_daos_md_rw_prep_cb, H5_daos_group_gmco_comp_cb, fetch_udata, &fetch_task) < 0)
