    ...
    hid_t H5VL_DAOS_UNUSED dxpl_id, void **req)
{
     ...
     */
    H5_DAOS_GET_METADATA_WRITE_MODE(item->file, tapl_id, H5P_DATATYPE_ACCESS_DEFAULT,
    ...
    tse_task_t **first_task, tse_task_t **dep_task)
{
    ...
    if(!collective || (file->my_rank == 0)) {
        ...
        if(NULL == (update_cb_ud = (H5_daos_md_rw_cb_ud_flex_t *)DV_calloc(sizeof(H5_daos_md_rw_cb_ud_flex_t) + type_size + tcpl_size)))
        ...
        daos_const_iov_set((d_const_iov_t *)&update_cb_ud->md_rw_cb_ud.iod[1].iod_name, H5_daos_cpl_key_g, H5_daos_cpl_key_size_g);
        ...
        if(parent_grp) {
            ...
            finalize_deps[finalize_ndeps] = *dep_task;
            if(0 != (ret = H5_daos_obj_write_rc(NULL, &dtype->obj, NULL, 0,
                    req, first_task, &finalize_deps[finalize_ndeps])))
    ...
    hid_t tapl_id, hid_t H5VL_DAOS_UNUSED dxpl_id, void H5VL_DAOS_UNUSED **req)
{
     ...
     */
    H5_DAOS_GET_METADATA_READ_MODE(item->file, tapl_id, H5P_DATATYPE_ACCESS_DEFAULT,
            collective, H5E_DATATYPE, NULL);
    ...
    H5_daos_req_t *req, tse_task_t **first_task, tse_task_t **dep_task)
{
     ...
     * info buffer size */
    if(collective && (file->num_procs > 1)) {
        if(NULL == (bcast_udata = (H5_daos_mpi_ibcast_ud_flex_t *)DV_malloc(sizeof(H5_daos_mpi_ibcast_ud_flex_t) + H5_DAOS_TINFO_BCAST_BUF_SIZE)))
    ...
    if(!collective || (file->my_rank == 0)) {
        ...
        daos_const_iov_set((d_const_iov_t *)&fetch_udata->md_rw_cb_ud.iod[1].iod_name, H5_daos_cpl_key_g, H5_daos_cpl_key_size_g);
        ...
        if(H5_daos_create_task(NULL, 0, NULL, NULL, NULL, NULL, &fetch_udata->fetch_metatask) < 0)
            D_GOTO_ERROR(H5E_DATATYPE, H5E_CANTINIT, NULL, "can't create meta task for datatype metadata read");
...
H5_daos_datatype_open_bcast_comp_cb(tse_task_t *task, void H5VL_DAOS_UNUSED *args)
{
    ...
    if(task->dt_result < -H5_DAOS_PRE_ERROR
        ...
        if(udata->bcast_udata.buffer_len != udata->bcast_udata.count) {
            ...
            if(H5_daos_create_task(H5_daos_mpi_ibcast_task, 0, NULL, NULL, H5_daos_datatype_open_bcast_comp_cb,
                    udata, &bcast_task) < 0)
                D_GOTO_ERROR(H5E_DATATYPE, H5E_CANTINIT, -H5_DAOS_SETUP_ERROR, "can't create task for second datatype info broadcast");
    ...
    if(udata) {
        /* Close datatype */
        if(udata->bcast_udata.obj && H5_daos_datatype_close_real((H5_daos_dtype_t *)udata->bcast_udata.obj) < 0)
        ...
        if(H5_daos_task_list_put(H5_daos_task_list_g, udata->bcast_udata.bcast_metatask) < 0)
            D_DONE_ERROR(H5E_DATATYPE, H5E_CLOSEERROR, -H5_DAOS_TASK_LIST_ERROR, "can't return task to task list");
...
H5_daos_datatype_open_recv_comp_cb(tse_task_t *task, void H5VL_DAOS_UNUSED *args)
{
    ...
    if(task->dt_result < -H5_DAOS_PRE_ERROR
        ...
        if(tinfo_len > (size_t)udata->bcast_udata.count) {
            ...
            /* Create task for second bcast */
            if(H5_daos_create_task(H5_daos_mpi_ibcast_task, 0, NULL, NULL, H5_daos_datatype_open_recv_comp_cb,
                    udata, &bcast_task) < 0)
                D_GOTO_ERROR(H5E_DATATYPE, H5E_CANTINIT, -H5_DAOS_SETUP_ERROR, "can't create task for second datatype info broadcast");
            ...
            if(0 != (ret = H5_daos_datatype_open_end((H5_daos_dtype_t *)udata->bcast_udata.obj,
                    p, type_buf_len, tcpl_buf_len, udata->bcast_udata.req->dxpl_id)))
    ...
    if(task->dt_result == -DER_REC2BIG) {
        ...
        if(udata->bcast_udata) {
            ...
            if(daos_info_len > H5_DAOS_TYPE_BUF_SIZE + H5_DAOS_TCPL_BUF_SIZE) {
                if(NULL == (udata->bcast_udata->bcast_udata.buffer = DV_malloc(daos_info_len + H5_DAOS_ENCODED_OID_SIZE + 2 * H5_DAOS_ENCODED_UINT64_T_SIZE)))
                    D_GOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, -H5_DAOS_ALLOC_ERROR, "can't allocate buffer for serialized datatype info");
                udata->bcast_udata->bcast_udata.buffer_len = (int)(daos_info_len + H5_DAOS_ENCODED_OID_SIZE + 2 * H5_DAOS_ENCODED_UINT64_T_SIZE);
            } /* end if */

            /* Set starting point for fetch sg_iovs */
            p = (uint8_t *)udata->bcast_udata->bcast_udata.buffer + H5_DAOS_ENCODED_OID_SIZE + 2 * H5_DAOS_ENCODED_UINT64_T_SIZE;
        } /* end if */
        else {
        ...
        if(H5_daos_create_daos_task(DAOS_OPC_OBJ_FETCH, 0, NULL, H5_daos_md_rw_prep_cb,
            ...
            D_GOTO_ERROR(H5E_DATATYPE, H5E_CANTINIT, -H5_DAOS_SETUP_ERROR, "can't create task to read datatype metadata");
...
H5_daos_datatype_close(void *_dtype, hid_t H5VL_DAOS_UNUSED dxpl_id, void **req)
{
    ...
    if(((dtype->obj.item.open_req->status == 0
        ...
        if(NULL == (task_ud = (H5_daos_obj_close_task_ud_t *)DV_calloc(sizeof(H5_daos_obj_close_task_ud_t))))
            D_GOTO_ERROR(H5E_DATATYPE, H5E_CANTALLOC, FAIL, "can't allocate space for close task udata struct");
        task_ud->req = int_req;
    ...
    if(int_req) {
        ...
        if(req) {
            ...
            *req = int_req;
            ...
            /* Check for failure */
            if(int_req->status < 0)
                D_DONE_ERROR(H5E_DATATYPE, H5E_CANTOPERATE, FAIL, "datatype close failed in task \"%s\": %s", int_req->failed_task, H5_daos_err_to_string(int_req->status));
