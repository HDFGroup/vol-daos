    ...
    switch (loc_params->type) {
        case H5VL_OBJECT_BY_NAME:
            if(H5_daos_object_open_by_name((H5_daos_obj_t *)item, loc_params, &oid, dxpl_id, req) < 0)
                D_GOTO_ERROR(H5E_OBJECT, H5E_CANTOPENOBJ, NULL, "can't open object by name");
            break;

        case H5VL_OBJECT_BY_IDX:
            if(H5_daos_object_open_by_idx((H5_daos_obj_t *)item, loc_params, &oid, dxpl_id, req) < 0)
                D_GOTO_ERROR(H5E_OBJECT, H5E_CANTOPENOBJ, NULL, "can't open object by index");
            break;
    ...
    daos_obj_id_t *opened_obj_id, hid_t dxpl_id, void **req)
{
        ...
        if(!strncmp(loc_params->loc_data.loc_by_name.name, ".", 2)) {
            ...
            if(target_name[0] == '\0'
                ...
                if((link_resolved = H5_daos_link_follow(target_grp, target_name, strlen(target_name), dxpl_id, req, &oid)) < 0)
                    D_GOTO_ERROR(H5E_OBJECT, H5E_TRAVERSE, FAIL, "can't follow link to group");
                if(!link_resolved)
                    D_GOTO_ERROR(H5E_OBJECT, H5E_TRAVERSE, FAIL, "link to group did not resolve");
            } /* end else */
        } /* end else */
    ...
    *opened_obj_id = oid;
    ...
    if(target_grp && H5_daos_group_close(target_grp, dxpl_id, req) < 0)
        D_DONE_ERROR(H5E_OBJECT, H5E_CLOSEERROR, FAIL, "can't close group");
    ...
    daos_obj_id_t *opened_obj_id, hid_t dxpl_id, void **req)
{
    ...
    if(!link_resolved)
        D_GOTO_ERROR(H5E_OBJECT, H5E_TRAVERSE, FAIL, "link to object did not resolve");
